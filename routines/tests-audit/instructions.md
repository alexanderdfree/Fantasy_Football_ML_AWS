...
1. Read the relevant rules in `agent-guides/stop-rules.md`, plus the testing contracts in `agent-guides/modeling.md`
   ...
   CI & training section in `agent-guides/operations.md` (`tests.yml` shard matrix, `pytest -m unit`, the 80%
   per-flag Codecov target, the `[docs-only]` opt-in). Keep these as scoped references for workers and final verification.
