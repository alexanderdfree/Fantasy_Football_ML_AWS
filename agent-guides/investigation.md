# Investigation and communication

<a id="verification-audit-communication"></a>
<a id="verification-audit--communication"></a>

## Verify behavior and effective state

- **Audit by running the test, not grepping.** "Every position satisfies Y" needs an actual run — symbol grep misses `LOSS_WEIGHTS=` vs `loss_weights=` kwarg forms. Check that a pass is not an all-skipped run: in the May 2026 audit behind this rule, a defensive `pytest.skip()` on the stale `LOSS_WEIGHTS` lookup skipped every position, so run such a test before and after turning the skip into an assert.
- **Check ALL config layers for "is X implemented?"** In Batch/ECS a per-submission `submit_job(...)` override can invalidate the resource default.
- **Changing an endpoint contract:** grep the route path AND the handler name (`def health`, `/health`), not just the data structures read.
- **Enumerate before filtering in tree-wide sweeps.** One unfiltered `grep -rn <term> .`, categorize EVERY hit, THEN filter — a `grep -v` as the sweep tool gives phantom completeness (#637 missed user-facing wiki refs).
- **Default investigation sweep target is the property-extremum case,** not the example the user happened to name.
- **Verify a library version / `requires-python` via `pypi.org/pypi/<pkg>/json`,** not install output.
- **File existence is filesystem, not git.** An untracked parent-checkout file is invisible to origin/main + the worktree path — `find` broadly before crying hallucination.
- **Trace artifact lifecycle.** Adding a file to a dir that's later uploaded? Trace every call between write→upload — a `_replace_*`/`_reset_*` may wipe the destination.
- **Don't act on garbled/truncated tool output** — re-verify with one clean sequential call before reporting a number (#622 cited a wrong σ + a false regen "success").
- **A skill/hook file existing ≠ it firing** — confirm the trigger path before reporting a mechanism "done."
- **A hand-rolled shell command-matcher/tokenizer that gates a destructive or PR action** (the `gh pr <subcmd>` matcher in [.claude/hooks/lib.sh](../.claude/hooks/lib.sh) / [.codex/hooks/lib.sh](../.codex/hooks/lib.sh), #893/#894) is safety-critical — pin it with a regression test in the `shared` pytest shard ([tests/hooks/test_pr_tokenizer.py](../tests/hooks/test_pr_tokenizer.py) / [tests/scripts/test_codex_hooks.py](../tests/scripts/test_codex_hooks.py), #1232/#1241), not manual review; document known gaps (e.g. heredoc bodies) inline.

## Scope and pending decisions

- **Surface gate friction with options** (eat the cost / authorized bypass / fix the gate) instead of silently bypassing — the user often picks "fix the gate."
- **Don't start a gated action while the decision is still open.** When the user's pending input is a go/no-go or which-approach gate, hold the irreversible/outward step (force-push, `workflow_dispatch`, Batch submit, merge) until the answer lands — a parallel dispatch outruns the decision and forces a walk-back/cancel (PR #1214: pushed the benchmark-infra fix + re-dispatched the GPU run before the "plan it, don't build yet" answer arrived). After a context resume, reconcile your own already-shipped commits / in-flight runs (`git log origin/main..HEAD`, `gh run list`) before re-issuing a pending question — your prior-turn actions may have overtaken it, making a verbatim re-ask stale. Distinct from "Surface gate friction" (CI false-positive) and "Validate a tentative mechanism" (design-fit).
- **Surface user-requested scope that a mid-implementation audit shows infeasible** — pause and offer options, don't unilaterally drop it (#273 dropped a requested item without asking; the pre-PR judge flagged it).
- **Goal governs over named mechanism.** A user may name a gate ("make pre-pr require signoff") but mean a goal ("signoff before merge"); two phrasings can be ONE checkpoint — confirm the mapping at plan time (#642).
- **Validate a tentatively-proposed mechanism before building.** A "maybe use X?" is a hypothesis, not a decision; for a *new subsystem* present a short architecture tradeoff (2–3 options + a recommendation) and confirm first. Matching existing patterns is a point *in favor*, not a reason to skip the comparison. (Distinct from "Goal governs" — there mechanism→goal; here the user names a mechanism tentatively and wants the design-fit validated. An S3 memory-sync plan built straight from the hint drew an explicit "confirm it's the right architecture first"; #686 shipped only after S3, a private git repo and file sync were compared.)

## Superseded guidance

- **"Did we remove/change X?" is two layers, not one.** A correct code/config state ≠ a clean repo — the change that landed X often updated *some* mirrors (ADR, CHANGELOG) but not all. `git grep -i <feature>` across `docs/`, `docs/adr/`, `TODO.md`, `todo/` for stale recommendations / active-tense prose; a "go add it" follow-up for a now-removed feature actively misleads the next contributor. Fix the harmful ones (PR + merge if repo-tracked); leave accepted convention alone (CHANGELOG `(PR pending)` markers). opp-def removal #1175 left `docs/rb_feature_history.md` recommending RB add the just-removed branch → caught only on a re-sweep (#1184).

Use [delivery](delivery.md#worktree-workflow) for worktree/remote-state checks and
[production validation](validation.md#production-path) for model/data evidence.
