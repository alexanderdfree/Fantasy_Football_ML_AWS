...
- **Promoting rolling / L3 / L5 / L8 / ewma / trend features into `ATTN_STATIC_FEATURES`** — see [the attention whitelist](modeling.md#attention-static-feature-whitelist-is-separate-per-position); the static branch is deliberately non-temporal. Not a way to "close the gap to LightGBM" — the gap is architecture, not input availability. A 2026-05-20 RB feature plan proposed this in four of five tasks and was rejected.
