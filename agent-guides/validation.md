...
**Evaluation cohorts (ADR-0024):** compare sources on identical regular-season player-weeks and the same projected scoring components in forecasts AND actuals (owner clarification, 2026-09-10). Use `src/shared/comparison_scoring.py`; missing component data is unavailable, never a full-fantasy fallback. NFL.com is never graded: its K totals lack our components, and its offense archive is RotoWire's projection series captured at uncontrolled times (10 of 18 2025 weeks predate the final injury report). The Comparison tab's headline cohorts are selected by no graded forecast: `weekly_depth_starters` (pregame depth chart) and `elite_top24` (prior-season importance). `weekly_reference_top24` (archived RotoWire; ESPN for K) is a secondary view, because selecting on a source's own forecasts penalizes that source. A row names a winner only when the paired, player-clustered bootstrap interval for the served model minus the best expert excludes zero under both MAE and RMSE (`src/shared/comparison_uncertainty.py`). A provider row with every published stat at zero is a missing forecast (roster placeholder), never a 0.0; Timeline groups match the tab's graded sources. The graded 2025 season is the A/B development season, so label it a development-season backtest. Actual weekly leaders are for ranking, and actual seasonal leaders are retrospective. Never use a model's own top-N pool for cross-source MAE comparison, restore the static expert summary as live accuracy, or silently omit missing cohort data from serialized Batch/local results. The row verdict grades the served model (`SERVED_MODEL` in `src/contracts/api.py`, the Next Week ranking chain) against the best expert; best-of-four is context only, and `top12`/`top30`/`weekly_reference_top24` carry no verdict.
