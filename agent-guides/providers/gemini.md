...
Start with [AGENTS.md](../../AGENTS.md), the shared router and core invariants. Read this reference only for the provider mechanisms relevant to the task. **This file adds only the Gemini-specific machinery**: skills, auto-memory, and operational differences from Claude Code and Codex. Update shared workflow behavior in `agent-workflows/` and the relevant topic guide; update provider adapters only where their mechanics differ.
...
## Auto-Memory & State
Gemini CLI / Antigravity manages state through plain Markdown memory files:
...
- **Durable Cross-Agent Project Facts:** Any fact, ML method, or Git workflow that applies to the entire team or multiple agents (Claude/Codex/Gemini) belongs in the relevant [agent guide](../README.md) or ADR, linked from `AGENTS.md`; keep private memory as a scoped retrieval pointer.
...
- **`pre-pr.sh`** — `BeforeTool` on `run_shell_command`: on a top-level `gh pr create`, delegates to the single-source gate `.claude/hooks/pre-pr.sh` (ruff + `pytest -m unit` + fingerprint-scoped benchmark evidence), exactly as the Codex wrapper does.
