...
class TestMultiHeadNet:
    ...
    def model(self):
        return MultiHeadNet(
            input_dim=10, target_names=QB_TARGETS,
    ...
    def test_custom_backbone(self):
        model = MultiHeadNet(
            input_dim=5, target_names=QB_TARGETS,
    ...
    def test_total_equals_sum_train_mode(self):
        """Total = sum of heads should hold in train mode too."""
        model = MultiHeadNet(
            input_dim=10, target_names=QB_TARGETS,
    ...
    def test_single_backbone_layer(self):
        ...
        model = MultiHeadNet(
            input_dim=10, target_names=QB_TARGETS,
            backbone_layers=[128], head_hidden=32, dropout=0.2,
        )
        model.eval()
        x = torch.randn(4, 10)
        with torch.no_grad():
