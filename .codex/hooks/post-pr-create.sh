...
ctx='PR opened. Continue with `/prompts:post-pr-followup` (template: `.codex/prompts/post-pr-followup.md`): rebase onto origin/main, run `codex review --base origin/main`, apply localized review fixes, wait for CI, then merge/delete the remote branch when allowed. The prompt preserves audit/tier explicit merge sign-off and the post-session-critique nudge.'
