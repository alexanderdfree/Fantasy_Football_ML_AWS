...
context="Fantasy_Football_ML_AWS Codex context: read AGENTS.md before non-trivial edits. This repo has tracked Codex hooks under .codex/hooks; review/trust them with /hooks after changes. Custom prompt wrappers live in .codex/prompts, shared cross-agent workflow behavior lives in agent-workflows, and Codex only loads user-home prompts, so run scripts/bootstrap-codex-local.sh and restart Codex when those templates change."
