...
7. Merge once CI is green and the step-6 gate is clear:
   - **`audit-*/tier-*` branches:** show `gh pr diff PR` (plus any Tier-C benchmark deltas) and ask for **explicit merge sign-off**; only after the user approves, run `gh pr merge PR --squash`, then `git push origin --delete <headRefName>`.
