...
7. Merge once CI is green and the step-6 gate is clear. A **pinned merge** merges only the head whose checks ran: confirm `gh pr view PR --json headRefOid,mergeStateStatus` shows `headRefOid` equal to `git rev-parse HEAD` and `mergeStateStatus` `CLEAN` (if not, push any local commits or wait for the current run and repeat step 5), then run `gh pr merge PR --squash --match-head-commit "$(git rev-parse HEAD)"`. If gh reports that the head moved, repeat step 5.
   - **`audit-*/tier-*` branches:** show `gh pr diff PR` plus any `regress-risk-high` benchmark deltas, and ask for **explicit merge sign-off**. After approval, do the pinned merge; a head that differs from the approved one needs fresh sign-off. Verify MERGED state and the latest reviewed squash content before separately running `git push origin --delete <headRefName>`.
   - **Other branches:** do the pinned merge. Verify MERGED state and latest squash content before `git push origin --delete <headRefName>`. If branch protection still blocks merging after the documented fallback, surface that gate; do not use `--admin`. Do not use `gh pr merge --delete-branch` from a worktree.
