...
5. Wait for green CI with `gh pr checks PR --watch`. If any check fails, stop and report the failing check; do not retry the merge or use `--admin`. Exception: the documented `Run Tests` silent-stop anomaly in `agent-guides/operations.md`; with user confirmation, run local `pytest` and then continue to the branch-specific merge/sign-off step below. This does not authorize bypassing branch protection or a different failing/pending check.
