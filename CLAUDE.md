...
- **`[docs-only]` commit-message opt-in for comment/docstring/import-reorder PRs.** When every change in the PR is non-behavioural (a comment text fix, a docstring update, an `is*` typo, ruff I001 import reordering) and you are 100% certain there is no metric or runtime impact, include `[docs-only]` in at least one commit message in the PR. The [tests.yml](.github/workflows/tests.yml) `detect` job sees the tag and emits an empty shard matrix, so the 7-shard test fan-out is skipped; `lint` + `detect` still run as cheap sanity gates and `tests-pass` reports green via the `skipped` branch. Trust contract — CI cannot verify the assertion, the author owns correctness. Don't reach for the tag to bypass a flaky test; if `[docs-only]` is wrong, the next substantive PR's CI will surface the regression with no signal about which commit introduced it.
