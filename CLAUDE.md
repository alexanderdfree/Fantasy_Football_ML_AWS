...
- **`[docs-only]` commit-message opt-in for comment/docstring/import-reorder PRs.** When every change in the PR is non-behavioural (a comment text fix, a docstring update, an `is*` typo, ruff I001 import reordering) and you are 100% certain there is no metric or runtime impact, include `[docs-only]` in at least one commit message in the PR. The tag is respected by:
  ...
  - [batch-image.yml](.github/workflows/batch-image.yml)'s `build-and-push` job — skips entirely (image rebuild is wasteful when runtime behaviour is unchanged).
