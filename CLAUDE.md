...
The NN clamps outputs to ≥ 0 per head. DST's `pts_allowed_bonus` legitimately ranges [-4, +10], so DST passes an explicit `non_negative_targets` set that excludes it. If you construct `MultiHeadNet(...)` anywhere outside `shared/pipeline.py::_train_nn`, mirror the kwarg — the CV path was missed once (see TODO.md archive).
...
- `tests.yml` — ruff + pytest on push/PR. Note the known issue in auto-memory: `Run Tests` can silently stop firing on rapid force-push cadence; run `pytest` locally and merge with `gh pr merge --squash` if CI looks stuck.
