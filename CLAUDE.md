...
- **After `gh pr create`, the [.claude/hooks/post-pr-create.sh](.claude/hooks/post-pr-create.sh) PostToolUse hook injects a follow-up workflow.** Claude's next turn (1) runs `git fetch origin main --quiet && git rebase origin/main` to fold in any newly merged PRs and force-pushes if the rebase changed anything (aborts + surfaces to user on conflict), (2) invokes the `/review` skill on the PR, (3) auto-applies nit/minor/style findings via Edit/Write, (4) surfaces major/blocker/behavioral findings for user judgment, (5) commits any nit fixes as `review: address /review nits` and pushes. The hook itself only injects the instruction — Claude executes the workflow. Pairs with the `pre-pr-judge` bullet above (pre/post around `gh pr create`).
