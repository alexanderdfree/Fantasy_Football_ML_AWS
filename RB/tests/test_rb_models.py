...
from RB.rb_config import RB_GATED_ORDINAL_TARGETS, RB_ORDINAL_TARGETS, RB_TARGETS
...
class TestRidgeMultiTarget:
    ...
    def test_total_is_sum_of_components(self, simple_ridge_data):
        X, y_dict = simple_ridge_data
        model = RidgeMultiTarget(target_names=RB_TARGETS, alpha=1.0)
        model.fit(X, y_dict)
        preds = model.predict(X)

        expected_total = sum(preds[t] for t in RB_TARGETS)
...
class TestRidgeMultiTargetWithGatedOrdinal:
    ...
    def test_construction_with_two_gated_td_targets(self):
        ...
        from shared.models import GatedOrdinalTDClassifier as _Gated
