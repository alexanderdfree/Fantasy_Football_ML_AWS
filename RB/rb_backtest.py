...
def run_rb_weekly_simulation(
    test_df: pd.DataFrame,
    pred_columns: dict,
    true_col: str = "fantasy_points",
    top_k: int = 12,
) -> dict:
    """Week-by-week simulation for RB models across the test season.
    ...
    for week in sorted(test_df["week"].unique()):
        ...
        for model_name, pred_col in pred_columns.items():
            ...
            # Ranking metrics (RB-only, no position filter needed)
...
def plot_rb_weekly_accuracy(sim_results: dict, save_path: str) -> None:
    """Two-panel figure: weekly MAE and top-12 hit rate for RB models."""
    ...
    ax1.set_xlabel("Week")
    ax1.set_ylabel("MAE")
    ax1.set_title("RB Weekly MAE by Model")
    ax1.legend()
    ...
    ax2.set_title("RB Weekly Top-12 Hit Rate")
    ax2.legend()
