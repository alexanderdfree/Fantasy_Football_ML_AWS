...
def print_table(all_results):
    print("\n" + "=" * 115)
    print(f"{'Label':<45} {'Total':>7} {'TD MAE':>7} {'Rush':>7} {'Recv':>7} {'R2':>6} {'Top12':>6} {'Time':>5}")
    print("-" * 115)
...
if __name__ == "__main__":
    ...
    if args.step == 0:
        ...
        best_w = float(best["overrides"].get("attn_td_gate_weight", "1.0")) if best["overrides"] else 1.0
        print(f"Best gate weight so far: {best_w} (TD MAE={best['attn_td_mae']})")
        for h in [24, 32]:
            row = run_variant(f"gate_hidden={h} (w={best_w})",
        ...
        best_w = float(best["overrides"].get("attn_td_gate_weight", "1.0")) if best["overrides"] else 1.0
        best_h = int(best["overrides"].get("attn_td_gate_hidden", "16")) if "attn_td_gate_hidden" in best.get("overrides", {}) else 16
        print(f"Best so far: w={best_w}, h={best_h} (TD MAE={best['attn_td_mae']})")
        for d in [2.0, 1.5]:
            deltas = {"rushing_floor": 2.0, "receiving_floor": 2.5, "td_points": d}
            row = run_variant(f"huber_delta={d} (w={best_w}, h={best_h})",
                              {"attn_td_gate_weight": best_w, "attn_td_gate_hidden": best_h,
                               "huber_deltas": deltas})
            all_results.append(row)
        ...
        for td_lw in [2.5, 3.0]:
            ...
            row = run_variant(f"td_loss_w={td_lw} (gw={best_w}, h={best_h}, d={best_d})",
                              {"attn_td_gate_weight": best_w, "attn_td_gate_hidden": best_h,
                               "huber_deltas": deltas, "loss_weights": lw})
            all_results.append(row)
