...
- **Status:** Lever A built/measured/shipped off by default for normal training; AWS Batch tune path wires graph-on + NVIDIA MPS as a tuner-only profile. Lever B remains planned only for local Windows/WSL where NVIDIA MPS is unavailable.
...
### [PRIORITY] Run batch-size + LR attention ablation harness (built, NOT yet run)
- **Harness:** [src/tuning/ablate_batch_lr.py](src/tuning/ablate_batch_lr.py), using the shared runner in [src/tuning/ablation_runner.py](src/tuning/ablation_runner.py). Tests: [tests/tuning/test_ablate_batch_lr.py](tests/tuning/test_ablate_batch_lr.py), [tests/tuning/test_ablation_runner.py](tests/tuning/test_ablation_runner.py).
- **What:** Implements the controlled answer to "can larger attention batches speed updates without repeating the historical accuracy regression?" The variants isolate batch-only (`b2_lr1`) from AdamW sqrt-scaled LR (`b2_lrsqrt`), aggressive linear LR (`b2_lrlin`), and a capped 4x/sqrt point (`b4_lrsqrt`). The harness changes only offline run configs: production `src/{pos}/config.py` is untouched.
- **How:** default is all six positions x eight paired seeds x five variants, attention-only with Ridge/ENet/base-NN/LGBM disabled for experiment runs. `--ridge-sentinel-preflight` adds one serial Ridge-enabled baseline vs `b2_lrsqrt` check per position to confirm data identity without paying Ridge CV cost on every run. The script records attention FP MAE, per-target MAE, min val loss, epochs, epoch timing, peak memory, and `phase_seconds["attn_nn_train"]`.
- **Command:** `python -m src.tuning.ablate_batch_lr --positions QB RB WR TE K DST --seeds 42,43,44,45,46,47,48,49 --ridge-sentinel-preflight`
- **Decision rule:** a variant is eligible only if paired attention FP MAE is no worse than baseline by 0.02 and not clearly worse than seed noise; recommend only the fastest eligible variant if it clears a 10% median `attn_nn_train` speedup. K is reported but not automatically recommended unless attention becomes competitive with the served model.
- **Status:** tooling built and dry-run/unit tested; full GPU experiment not run yet. No production config change until the harness produces a validated winner.
