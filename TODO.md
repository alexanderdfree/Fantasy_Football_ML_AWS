...
- **Files:** `src/k/data.py` (`reconstruct_kicker_weekly_from_pbp` venue backfill from XP plays, `load_data` schedules-based venue fallback with OAK/SD/STL → LV/LAC/LA remap, `_backfill_2025_pbp_columns` per-game venue lookup split into a second pass keyed on `(season, week, recent_team)`), `tests/k/test_data_loaders.py` (new `test_reconstruct_weekly_from_pbp_xp_only_game_gets_venue`, `test_load_data_backfills_venue_for_xp_only_games`, `test_load_data_treats_empty_string_surface_as_missing`; updated `test_backfill_2025_pbp_columns_updates_in_place` fixture to include `recent_team`).
