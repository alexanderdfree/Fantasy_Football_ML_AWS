...
- **Levers:** (A) **CUDA graphs** — built behind opt-in `FF_CUDA_GRAPH=1`; speed win is real, graph-vs-graph is clean, and graph-vs-eager is not bit-comparable. (B) **single-process + per-position CUDA streams** — the MPS substitute; bigger refactor that would supersede the subprocess model + the core pool.
- **Recommended:** CUDA graph benchmarkability is settled: use graphed-vs-graphed local rebaselines for `FF_CUDA_GRAPH=1`, not eager baselines. Consider Lever B only if the remaining local wall-clock bottleneck justifies the larger stream/orchestrator refactor.
