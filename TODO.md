...
- **Decision:** graphed-vs-graphed local rebaselining is the only clean fast A/B path. Lower fixed-scale tests do **not** make graph-vs-eager bit-comparable: `init_scale=2048` and `1024` still overflowed in the graph arm, while `512` completed with identical graph/eager scale traces but still produced a fixed-step RB attention MAE delta (`graph=4.044157`, `eager=4.060941`, `Δ=-0.016784`).
- **Status:** analysis harness implemented (`src/analysis/cuda_graph_gradscale.py`) and run locally on RB / RTX 5080. Graph-vs-graph is clean; BN warmup restore is inert; eager-vs-graph `GradScaler` schedules diverge at step 1; fixed-scale controls with identical scale schedules still drift. No production default change.
...
- **Judge on the subgroup metric** (`rookie_early` bias + bias-corrected MAE + top-12), **NOT** overall benchmark MAE — that will stay ~flat by construction (~0.05 overall) and reading it as failure is the #519 trap. Validate with `python -m src.analysis.cohort_analysis rookie --positions QB --with-model-error` before/after, ≥2 seeds.
...
- **Findings (read first):** [todo/rookie_cohort_findings_priority.md](todo/rookie_cohort_findings_priority.md) — per-position tables (rookie vs veteran MAE, signed bias by `rookie_early`/`rookie_rest`, bias-corrected MAE). Metric: [src/analysis/cohort_analysis.py](src/analysis/cohort_analysis.py) `rookie` cohort (read-only diagnostic, no retrain — `python -m src.analysis.cohort_analysis rookie --with-model-error`; legacy wrapper remains at `src.analysis.rookie_cohort_metrics`).
