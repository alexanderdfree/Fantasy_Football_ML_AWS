...
**Evidence:** Regression cases cover malformed payloads, 304 responses, interrupted streams, upload retry exhaustion, timestamp retention, previous-version recovery and resumption, worker exclusion, fork-inherited descriptors, real truncated HTTP responses, filesystem failures and browser visibility. Read-only production rehearsal uses the actual six-position, 519-row-per-format artifact; no feature/model recipe changes are required. CI exposed an existing cache-invalidation fixture whose parallel model stub omitted the production results-write lock; the fixture now uses that lock to avoid lost pandas updates.
