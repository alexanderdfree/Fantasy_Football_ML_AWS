...
**Follow-up update (2026-06-01):** first RB run of [cuda_graph_gradscale_ab_priority.md](cuda_graph_gradscale_ab_priority.md) is complete. Graph-vs-graph is clean (identical metrics + scale schedule), BN warmup snapshot/restore is inert, and eager-vs-graph first diverges in `GradScaler` at step 1. Fixed normal-scale mode is invalid because the default initial scale 65536 overflows at step 0. The practical decision is now narrower: accept graphed-vs-graphed local rebaselining, or test a lower explicit fixed initial scale as a new experimental mode with its own quality/speed gate.
