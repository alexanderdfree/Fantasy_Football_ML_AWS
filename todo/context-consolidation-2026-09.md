...
|---|---|
| `gh_pr_merge_worktree`, `feedback_no_verify_merges`, `feedback_squash_merge_verify_content`, `feedback_never_chain_destructive_action_past_a_gate` | [PR and merge gates](../agent-guides/delivery.md#pr-and-merge-gates), which gained the `--delete-branch` recovery, hook-gated command isolation, PR-number derivation and head-SHA checks; incidents in the [gate-chaining](fixed-archive/gate-chaining-closed-prs-2026-05.md) and [#292](fixed-archive/pr-292-merged-without-late-fix.md) records |
| `feedback_verify_checkout_before_rebase`, `feedback_verify_no_conflict_markers`, `feedback_check_open_prs_same_area`, `feedback_check_origin_main_in_worktree` | [Worktree workflow](../agent-guides/delivery.md#worktree-workflow); incidents in the [rebase-resolution record](fixed-archive/rebase-resolution-mistakes-2026-05.md) |
...
| `feedback_no_rolling_in_attn_static` | [Attention whitelist](../agent-guides/modeling.md#attention-static-feature-whitelist-is-separate-per-position) and [modeling stop rules](../agent-guides/stop-rules.md#modeling-and-features); the memory's `2.0/δ` loss-weight advice predated the MSE `1/δ` rule |
...
| Six worktree memories, merged into one Claude-harness memory | [Environment](../agent-guides/environment.md) and [worktree workflow](../agent-guides/delivery.md#worktree-workflow); history in the [parent-checkout record](fixed-archive/worktree-parent-checkout-edits-2026-05.md). Superseded advice to delete `data/raw` before linking and to treat Miniforge as the gate interpreter was dropped |
| Four CI-trigger memories, merged into one | [CI operations](../agent-guides/operations.md#ci-training) and [PR and merge gates](../agent-guides/delivery.md#pr-and-merge-gates); the old `--admin` fallback was dropped because both forbid it |

References to retired memory names in the shared solve-issues workflow and in
two `src/tuning` docstrings now point to these guides. The auto-merge guidance
is conditional on the repository setting, confirmed disabled on 2026-09-25.
