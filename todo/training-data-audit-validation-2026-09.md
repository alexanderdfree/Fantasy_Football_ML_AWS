...
Fresh native **K and D/ST seed-42 runs at `78b098ea`** also passed with unchanged pre/post code and input fingerprints. Their complete saved frames match the original fixed runs exactly: K has 543 rows and 21 prediction columns; D/ST has 544 rows and 45 prediction columns. Every prediction delta is zero. These two additional runs refresh the content-fingerprint gate without replacing the original three-seed evidence. See the [fresh canonical record](../benchmark_history/2026-09-10T14-46-00_78b098ea_training_data_fix_seed42.json) and local `review_refresh_outputs/forecast-equivalence.json`.

## Integration with model publication and immutable prediction caches

Before merging, the branch was rebased onto PR #1560 (`abe8960c`). The integration preserves immutable prediction-cache generations and model publication ordering while applying the data-release checks to the actual resolved image. Deployment prepares its prediction cache with the same selected data release before rollout; EC2 upgrades preserve image digests; manual launches register and report the resolved source SHA.

At integrated source `c41cccc4`, all six native preparation paths again produced identical feature, target, complete-frame, feature-list, and configuration hashes against the validated fixed inputs, with source fetching forbidden. Two additional K/DST seed-42 production runs passed; both complete prediction Parquet files are byte-identical to the original fixed runs. Pre/post source and input fingerprints matched, and the benchmark gate passed. See the [integration benchmark record](../benchmark_history/2026-09-10T19-42-17_c41cccc4_training_data_fix_seed42.json), local `merge-preparation-equivalence.json`, and `merge_refresh_outputs/forecast-equivalence.json`.

The combined unit suite passed **4,205 tests, with 2 skipped**. Integration tests retain real immutable-image resolution and ancestry checks, enforce registration before submissions, preserve pinned data IDs, and verify that cache rebuilding cannot regenerate or modify the sealed reference.

The subsequent integration with PR #1559 (`a5fa4761`) also preserves immutable, complete-run History publication. History descriptors and both summary levels retain the data release; mismatched or unknown snapshots cannot enter a pinned run, while legacy unknown records remain readable. Workflow attempts use separate split staging. All six numerical code fingerprints remain identical to the validated `60413a75` tree, so the **40 completed production runs** remain applicable. The final combined unit suite passed **4,241 tests, with 2 skipped** at `51ac9d5e`.
