...
**Status:** DONE (2026-06-19). Ran the grid — RB stacked N=24 (`ab_attn_arch-20260615T072824Z-b5b46ea`) for the six vmap-safe flags, plus eager RB+QB 8-seed for `selfattn` (`selfattn-confirm-15d827c`, via the new `launch_ablate` Batch path, #1180). **On MAE, no flag beats baseline.** A follow-up **RMSE re-screen** (`condq-screen-stacked-80494dd` + `-kdst`, 2026-06-20 — the eager extractor had been dropping RMSE) then found `condq` (#121) MAE-flat but tail-trimming → **ENABLED for RB + WR + TE** (PR #1198; RB attn-NN RMSE −0.051±0.028, 24/24 clean win, TE tail gain at a small median cost, **WR a forward-bet** — WR regresses on the screen but that's a CB-level matchup-feature data gap, not a broken mechanism, #1210; QB/K/DST stay OFF; see [../docs/adr/0004-attention-over-game-history.md](../docs/adr/0004-attention-over-game-history.md)). The other six stay default-OFF; scaffolding **kept** (no deletion). Full tables in [fixed-archive.md](fixed-archive.md). Only `entropy` remains un-run (eager-only vmap side-channel). Nothing to pick up.
