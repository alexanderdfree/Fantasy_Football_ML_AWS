...
> `[PRIORITY]` entry at the top of [TODO.md](../TODO.md)'s Open section.
...
1. **Optimizer is AdamW** ([`src/shared/pipeline.py:213`](../src/shared/pipeline.py)), loss is **mean-reduced** (`MultiTargetLoss` in [`src/shared/training.py`](../src/shared/training.py)). The linear LR-scaling rule (Goyal et al.) is an SGD result and does **not** apply. For Adam-family the grounded heuristic is **√-scaling**: `lr_new = lr_old · √(B_new / B_old)`.
2. **Batch size and LR are already *jointly* searched by Optuna** ([`src/tuning/tune_nn.py:179-186`](../src/tuning/tune_nn.py)): `nn_batch_size`/`attn_batch_size ∈ {128,256,512}`, `nn_lr`/`attn_lr ∈ [1e-4, 5e-3]` (log). The TPE sampler learns the batch↔LR coupling empirically — strictly better than hardcoding a √-rule. So **"adjust LR accordingly" = widen the LR ceiling so the larger-batch optimum isn't clipped**, not "apply a formula." (The √-rule is the fallback only if you ever hard-pin batch in `POSITION_CONFIG` instead of searching.)
...
**1. Extend both batch grids** in [`src/tuning/tune_nn.py`](../src/tuning/tune_nn.py) `_sample_overrides` (lines 180, 186):
```python
...
## Critical files
- [src/tuning/tune_nn.py](../src/tuning/tune_nn.py) — `_sample_overrides` (grids + LR range + comment).
