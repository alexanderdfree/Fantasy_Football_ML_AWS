...
A direct scrape of the actual live pages overturned the original rank-1 recommendation.

- **F1 FantasyPros is NOT a usable ≥2013 benchmark** (the original 0.81 screening score is retained below). Its historical pages (`/nfl/projections/{pos}.php?year=YYYY&week=W`) return real historical projected *values* but a player pool **filtered to players still rostered in the current (2025–26) season, labeled with their current teams**. The 2013 wk1 QB page returns only **~9 QBs** (Rodgers→"PIT", Stafford→"LAR", Cousins, Keenum, Tyrod Taylor…) — the entire since-retired 2013 cohort (Brees, Manning, Rivers, Roethlisberger) is **absent**. The original skeptic spot-checked a single still-active player (Rodgers), saw a genuine "Sep 5, 2013" value, and missed the pool truncation. Scoring the model against this decimated, survivorship-biased pool would be meaningless. **FantasyPros is valid only as a *current-season* consensus benchmark — it does not serve the ≥2013 goal.**
- **F3 FFToday has a genuine ≥2013 archive.** Its 2013 wk1 pages carry the **correct historical teams** (Calvin Johnson→DET, Dez Bryant→DAL, Demaryius Thomas→DEN, Wes Welker→DEN, Reggie Wayne→IND) and **include since-retired players** — a real, complete historical pool (a current-only page could not list retired players). Coverage: QB/RB/WR/TE/K (no 2013 DST); the table carries separate Team/Opp columns + per-position stat columns; archive floor 2010.

**Completed benchmark work:** the [FFToday loader](../src/analysis/fftoday_loader.py) shipped in #1376; full-archive vetting and comparison publication completed in #1477. The [expert-edge plan](expert-edge-action-plan.md) retains those results and the pending iso-edge extension. The feature findings (B1 CPOE / B2 PROE-pace / B3 opp-adj-EPA / E2 O-line continuity / H1 injury body-part text) and the coverage-data negative result are unchanged.

## 1. Executive summary

- **Remaining feature-screen candidates** derive from already-ingested data: **B1 CPOE**, **B2 PROE/pace**, **B3 opponent-adjusted EPA**, **E2 O-line continuity**, and **H1 injury body-part TEXT**. The dated source checks and screening scores are retained below.
- **Best pick per remaining lever:** *RB workload-derivative* → **B2 PROE/pace** (team pass/play-volume environment, the highest-confidence ordering re-sorter among the build-not-buy features; E2 O-line continuity is the orthogonal runner-up). *WR/TE coverage* → **no viable source** (see headline below).
...
### F1 — FantasyPros original page checks (2026-06-30; historical-pool candidate rejected)

The original screening assigned FantasyPros a score of 0.81 from the page checks below. The correction above rejected the historical pool after checking retired-player coverage; these observations do not establish a usable ≥2013 benchmark.

**Original page check.** The page accepted `year`/`week` for old seasons: `https://www.fantasypros.com/nfl/projections/qb.php?year=2013&week=1&scoring=STD` renders "2013 Week 1, Consensus of 7 Sources, Sep 5 2013" with per-player FPTS (Rodgers 21.0, Stafford 19.3, Dalton 15.5); `rb.php?year=2013&week=10` shows "2013 Week 10, Nov 9 2013".

**Original backfill check.** The 2013 Rodgers projection (291.4 yd / 2.2 TD) vs his realized 333/3 is a large miss — no ≤0.5-pt hvpkod backfill match — and the Sep-5 / Nov-9 2013 consensus dates are pre-kickoff. `backfill_risk = low`.
...
**Implemented:** [fftoday_loader.py](../src/analysis/fftoday_loader.py) supplies the internal benchmark through `analysis_expert_comparison._build_experts` (#1376); vetting and publication completed in #1477. It remains benchmark-only, not a feature.
...
## 5. Remaining feature-screen proposals
...
- **Vet genuinely new benchmark backfills before using them.** Require a full per-season hvpkod-style near-exact-match scan (`_LOOKAHEAD_EPS=0.5`); sampled page checks alone do not establish validity. FFToday's full-archive vet is [complete](expert-edge-action-plan.md#1b-fftoday-as-the-third-expert-benchmark-analysis-only); the FantasyPros historical pool remains rejected for survivorship bias.
- **Scraping ToS:** both surveyed projection sources are `license_redistribution = restricted`. FantasyPros' API access is personal/non-commercial with attribution and restricted redistribution; FFToday is a free public scrape but redistribution-restricted. Use them as **internal benchmarks only**, add rate-limit/sleep + caching, and do not redistribute the projections. Use the working `.php`/`playerwkproj.php` HTML endpoints — not the 403-ing `api.fantasypros.com/v2` path.
