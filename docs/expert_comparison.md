...
>   final injury report.
> - Provider rows with every published stat at zero are missing forecasts, not
>   zeros.
> - A row names a winner only when the served model's paired, player-clustered
>   bootstrap interval against the best expert excludes zero under both MAE and
>   RMSE; best of four is context. On the 2025 slate replayed at this audit
>   (generation `a475355f`, 2026-09-25) one row is decided, WR all rows, where
>   the served LightGBM trails the best expert on both metrics; every other row
>   is a tie.
...
- **NFL.com, Sleeper/RotoWire, FFToday and ESPN** provide per-game projections scored directly below. The Comparison tab grades RotoWire and ESPN; NFL.com offense is a stale copy of RotoWire's series and is listed with its exclusion reason rather than graded; FFToday remains an internal benchmark.
- **Training loss ≠ evaluation metric.** Our neural nets' yards heads train on MSE (switched from Huber in #870 — Huber's flat large-error gradient under-projected the elite tail), their count heads (TDs, INTs, fumbles) on Poisson NLL, and RB/WR/TE receptions on a zero-truncated negative-binomial hurdle; Ridge and LightGBM fit plain squared error. Every comparison here, though, is on MAE / RMSE / R² over held-out predictions — metrics that don't depend on what any model trained on. Squared error is the consistent scoring rule for the conditional *mean* (which MSE-trained heads and mean-oriented expert projections both target) and absolute error for the conditional *median* (Gneiting 2011), so the MAE we report rewards a functional none of these models was fit to — that is why the training loss and the evaluation metric differ, and why every source is scored head-to-head on the same MAE / RMSE rather than on any model's own objective (Huber, for the record, is consistent only for an intermediate robust "Huber mean" functional — Taggart 2022 — so nothing here is scored with it either). We report MAE (the better-matched summary for heavy-tailed errors) and RMSE side by side — RMSE because it favors the experts' implicit squared-error objective, which makes beating them on it the stronger claim.
...
**Dated research snapshot (2026-07-06); FFToday is not graded on the live tab.** Against the weakest of the three experts, the model **tied QB and RB** (CIs span zero), **leaned better on WR** (Δ −0.14, not significant), and **beat FFToday on TE** (CI excludes zero, DM p = 0.03, one test among many with no multiplicity correction) — the model's first statistically significant same-sample win over a human expert at a skill position. The expert-strength ordering this implies (NFL.com ≈ RotoWire, which share one projection series, > FFToday) is consistent with the per-expert verdicts above; on the current served artifacts the model sits at MAE parity with NFL.com and RotoWire and ahead of FFToday at TE (with a WR lean), which sharpens the roadmap's focus: the gap left to close is specifically the *stronger* experts' RB/WR tail + ordering edge.
...
K is interesting because kicker fantasy production is *highly* opponent/weather/coach dependent — and our model beat NFL.com's bucket-scored kicker total here (4.008 vs 4.358 MAE), a total ADR-0024 now excludes as not comparable; against ESPN's kicker forecast (near-constant field-goal counts, so a weak baseline) the served Ridge is ahead on MAE alone, a statistical tie on the live tab, after the `fg_yards_made` schema fix in [#227](https://github.com/alexanderdfree/Fantasy_Football_ML_AWS/pull/227) restored the FG-distance signal that had been silently dropped from the PBP cache. K's R² hovers near zero across all four models (−0.10 to 0.02); on this position, "low MAE + near-zero R²" is the right operating point — kicker variance week-to-week is too high to extract a meaningful rank-order signal, but absolute prediction error is tight.
