...
**References.** [src/shared/neural_net.py](../src/shared/neural_net.py) — `GatedTDHead` (L257, alias for `GatedHead` at L207), `AttentionPool` (L320), `MultiHeadNetWithHistory` (L524, outer-only attention used by QB/RB/WR/TE/DST), `MultiHeadNetWithNestedHistory` (L887, inner-kick ⊕ outer-game attention used by K); [src/k/config.py:102-152](../src/k/config.py) (nested attention, per-kick stats, static allowlist); [src/dst/config.py:169-228](../src/dst/config.py) (DST attention + history stats + static allowlist); [docs/archive/design_lstm_multihead.md](archive/design_lstm_multihead.md). Evolved across commits `b31bdf7` → `c399c12` → `99d7086` → `cc0c627` (DST attention + raw-stat targets) → `801b61a` (K nested attention).
...
| Six Spot g4dn.xlarge, parallel (chosen for `BATCH_ACTIVE=true`) | ~15 min (measured 2026-05-21; original design estimate ~25–30 min) | ~$0.40 (Spot × ~30 min × 6) | Yes — no `concurrency:` gate; runs coexist |
...
3. **No lineup optimizer** — out of scope for this ADR; tracked as a follow-up project, not a revision of D1–D15.
