...
**Chosen rationale.** AWS Batch was the original architecture ([docs/batch_design.md](batch_design.md)) before the pivot to warm EC2 cited cold-start dominating a 2-min training job (D7). Two factors closed that gap: (a) the cold-start blocker is partially mitigated by the SOCI index publish + `.dockerignore`/`COPY` discipline wired into [batch-image.yml](../.github/workflows/batch-image.yml); design target was ~60–90s, **measured 2026-05-20 was ~258s** (SOCI snapshotter inactive on the default AMI — see [docs/batch_design.md §2a](batch_design.md)), with ~135s achievable once the snapshotter is enabled; (b) more importantly, *parallelism dominates cold-start at six positions* — paying ~258s of cold-start once per Spot host while six positions train concurrently (~378s end-to-end including ~2 min training) is still much cheaper than paying ~0s of cold-start six times in sequence on one host (~120 min total wall-clock). The argument holds even at the measured (not designed) cold-start; activating the snapshotter would tighten it further. The Batch scaffolding (`src/batch/launch.py` with ThreadPoolExecutor + Spot-aware retry + `describe_jobs` polling) was 80% built before the EC2 pivot; this decision completes it.
...
**References.** [gunicorn.conf.py](../gunicorn.conf.py) (`post_fork` daemon thread), [src/serving/app.py](../src/serving/app.py) (`_PREDICTIONS_CACHE_DIR`, `_compute_models_fingerprint`, `_try_hydrate_from_disk`, `_persist_cache_to_disk`; hydrate wired into `_ensure_metrics`; persist wired into `_compute_metrics_locked`), [src/shared/model_sync.py](../src/shared/model_sync.py) (`sync_predictions_cache_from_s3`, `upload_predictions_cache_to_s3`), [Dockerfile](../Dockerfile) (CMD switched to `-c gunicorn.conf.py`, `COPY gunicorn.conf.py` added), [tests/test_app_predictions_cache.py](../tests/test_app_predictions_cache.py) + [tests/shared/test_model_sync.py](../tests/shared/test_model_sync.py) (`test_predcache_*`), [TODO.md](../TODO.md) archive ("First-request cold start" entry) and predecessor ("Gunicorn `--preload` pre-warm broke ALB health checks" entry — PRs #148/#149).
