...
### New function: `_train_xgboost()` in `shared/pipeline.py`
...
### Changes to `run_pipeline()` in `shared/pipeline.py`
...
### Changes to `_apply_position_models()` in `app.py`
...
### Changes to `POSITION_REGISTRY` in `app.py`
