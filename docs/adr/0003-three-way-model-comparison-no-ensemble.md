...
- **2026-09-10** — Honor the configured LightGBM `subsample` fraction by enabling per-iteration row sampling for fractions below one. Earlier code and Optuna trials supplied a fraction but left LightGBM's sampling frequency at zero, so the fraction had no effect. Whole-data fraction one retains its existing behavior. Corrected studies use `seedavg_bagging_v2`; historical tuned fractions are not evidence of quality with sampling enabled. This is an all-position model change requiring a fresh paired production comparison; independent model reporting is preserved. See [model audit](../../todo/fixed-archive/model-data-tuning-audit-2026-09.md).
