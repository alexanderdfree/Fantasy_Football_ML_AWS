...
| Per-position `ff-train` helper on PATH | `train-ec2.yml` fires one SSM command that loops through the six positions sequentially | Keeps the rollback path within one T4's memory envelope |
