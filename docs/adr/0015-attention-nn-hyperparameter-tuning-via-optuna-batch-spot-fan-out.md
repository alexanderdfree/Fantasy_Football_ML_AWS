...
- **Shared PostgreSQL Optuna storage** (one study across hosts, true parallel sampling): operational overhead (RDS instance, connection strings, race-condition handling) unjustified for the personal-project scale. Per-position SQLite + S3 checkpoint gives the resilience without the moving parts.
...
- **2026-06-01** — Added the issue #720 attention-knob experiment runner in [src/tuning/attn_knob_experiments.py](../../src/tuning/attn_knob_experiments.py): a test-MAE fANOVA path over eight attention-only knobs and a 12-run Plackett-Burman main-effects screen, both operator-triggered and history-recorded under tuning artifacts. This is tuner-only research tooling; no production `POSITION_CONFIG`, model, feature, target, or loss path changes.
