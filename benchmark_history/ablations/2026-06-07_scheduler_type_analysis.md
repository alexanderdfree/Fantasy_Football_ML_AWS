...
## Run scope
- Positions: QB, RB, WR, TE, K, DST (one Spot g6.xlarge / L4 per position, AWS Batch)
...
- **Eager**: `FF_CUDA_GRAPH=0` so the A/B is bit-comparable (CUDA graphs autodetect ON
  for sm_80+ but are not numerically inert — ADR-0017). Absolute MAEs therefore differ
  slightly from graphed production; the **type comparison (relative) is what's valid**.
...
- **`plateau` is unreachable from `PositionConfig`** (no `plateau_factor`/`plateau_patience`
  ...
  1σ result.
- Eager (graphs-off) for comparability; re-confirm a candidate under the production
  graphed path before changing config.
