"""Tests for src.shared.models.RidgeMultiTarget (using RB raw-stat targets)."""
...
class TestRidgeMultiTarget:
    ...
    def test_predict_total_matches(self, simple_ridge_data):
        ...
        preds = model.predict(X)
        np.testing.assert_allclose(total, sum(preds[t] for t in TARGETS), atol=1e-6)

    def test_different_alphas(self, simple_ridge_data):
        """Different regularization strengths produce different predictions."""
        X, y_dict = simple_ridge_data
        m1 = RidgeMultiTarget(target_names=TARGETS, alpha=0.01)
        m2 = RidgeMultiTarget(target_names=TARGETS, alpha=100.0)
        m1.fit(X, y_dict)
        m2.fit(X, y_dict)
    ...
    def test_save_and_load(self, simple_ridge_data, tmp_path):
        ...
        model2 = RidgeMultiTarget(target_names=TARGETS, alpha=1.0)
    ...
    def test_per_target_alpha_construction(self, simple_ridge_data):
        ...
        X, y_dict = simple_ridge_data
        alpha_dict = {t: 1.0 for t in TARGETS}
        alpha_dict["rushing_yards"] = 0.01
        alpha_dict["receiving_yards"] = 100.0
        m_per = RidgeMultiTarget(target_names=TARGETS, alpha=alpha_dict)
        m_uniform = RidgeMultiTarget(target_names=TARGETS, alpha=1.0)
        m_per.fit(X, y_dict)
        m_uniform.fit(X, y_dict)
        p_per = m_per.predict(X)
        p_uniform = m_uniform.predict(X)
        any_different = any(not np.allclose(p_per[t], p_uniform[t]) for t in TARGETS)
    ...
    def test_per_target_alpha_save_load(self, simple_ridge_data, tmp_path):
        ...
        model = RidgeMultiTarget(target_names=TARGETS, alpha=alpha_dict)
        model.fit(X, y_dict)
        preds_before = model.predict(X)
        ...
        model2 = RidgeMultiTarget(target_names=TARGETS)
        model2.load(model_dir)
        preds_after = model2.predict(X)
    ...
    def test_alpha_dict_missing_key_raises(self):
        ...
        partial = {t: 1.0 for t in TARGETS[:2]}
        with pytest.raises(ValueError, match="missing keys"):
            RidgeMultiTarget(target_names=TARGETS, alpha=partial)
...
class TestOrdinalTDClassifierRawCounts:
    ...
    def test_uses_config_from_rb_ordinal_targets(self):
        ...
        for target, cfg in ORDINAL_TARGETS.items():
...
class TestGatedOrdinalRawCounts:
    ...
    def test_gated_ordinal_targets_shape(self):
        """Both TD targets retargeted with class_values=[0,1,2,3]."""
        assert set(GATED_ORDINAL_TARGETS) == {"rushing_tds", "receiving_tds"}
        for cfg in GATED_ORDINAL_TARGETS.values():
...
class TestRidgeMultiTargetWithGatedOrdinal:
    ...
    ordinal classifier per ``GATED_ORDINAL_TARGETS``."""
