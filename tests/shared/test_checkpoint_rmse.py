...
class RecordingPlateau(torch.optim.lr_scheduler.ReduceLROnPlateau):
    """Plateau scheduler that records every metric value the trainer feeds it."""
    ...
    def step(self, metrics, epoch=None):
        self.seen.append(float(metrics))
...
@pytest.mark.parametrize("patience", [1, 20], ids=["early-stop", "epoch-limit"])
def test_fantasy_selector_restores_ppr_winner_but_keeps_loss_driven_hooks(patience):
    # The per-stat score favors one TD error over two reception errors;
    ...
    assert history["val_selection_metric"] == [6.0, 2.0, 3.0]
...
def test_default_selector_is_loss_weighted_mae():
    ...
    assert history["checkpoint_selection"]["metric"] == "weighted_mae"
    assert history["checkpoint_selection"]["scoring_format"] is None
    assert history["val_selection_metric"] == history["val_mae_weighted"]
...
def test_weighted_mae_restores_legacy_winner_when_rmse_prefers_another(patience, monkeypatch):
    ...
    # Epoch 1 has the lower MAE (2.0 vs 2.5) but the higher RMSE (sqrt(12) vs 2.5).
    assert history["val_mae_weighted"][:2] == pytest.approx([2.0, 2.5])
    assert np.argmin(history["val_rmse_weighted"]) == 1
    assert trainer.best_val_metric == pytest.approx(2.0)
    assert trainer.model.epoch.item() == 0
    assert history["val_selection_metric"] == history["val_mae_weighted"]
    selection = history["checkpoint_selection"]
    ...
    assert selection["score"] == pytest.approx(2.0)
