...
def test_ridge_keeps_special_heads_in_every_candidate_score(monkeypatch):
    cfg = dict(get_config("RB"))  # copy before mutating the shared recipe
...
def test_legacy_ridge_metric_uses_the_independent_search_and_records_nothing(monkeypatch):
    def boom(*args, **kwargs):
        raise AssertionError("legacy Ridge selection must not call the PPR search")

    monkeypatch.setattr(selection, "tune_ridge_ppr", boom)
    rng = np.random.default_rng(0)
    X = rng.normal(size=(24, 3))
    y = {"receptions": np.maximum(0, 3 + X[:, 0] + rng.normal(scale=0.1, size=24))}
    splits = np.repeat([1, 2, 3, 4], 6)
    grids = {"receptions": [0.1, 1.0, 10.0]}
    common = dict(n_cv_folds=2, n_jobs=1)
    reference = _tune_ridge_alphas_cv(X, y, splits, ["receptions"], grids, **common)
    info = {}
    legacy = _tune_ridge_alphas_cv(
        X,
        y,
        splits,
        ["receptions"],
    ...
    assert info == {}
    with pytest.raises(ValueError, match="Unknown Ridge selection metric"):
        _tune_ridge_alphas_cv(
            X, y, splits, ["receptions"], grids, cfg={"ridge_selection_metric": "bogus"}, **common
        )


def test_legacy_lightgbm_selection_keeps_per_target_early_stopping_layout(tmp_path):
    rng = np.random.default_rng(3)
    X = rng.normal(size=(48, 3))
    targets = get_config("K")["targets"]
    y = {t: np.maximum(0, 10 + (i + 1) * X[:36, 0]) for i, t in enumerate(targets)}
    y_val = {t: np.maximum(0, 10 + (i + 1) * X[36:, 0]) for i, t in enumerate(targets)}
    params = dict(
        n_estimators=8,
        num_leaves=4,
        min_child_samples=2,
    ...
    model = LightGBMMultiTarget(targets, **params)
    assert model.selection_metric == "per_target"
    model.fit(X[:36], y, X[36:], y_val, feature_names=["a", "b", "c"])
    assert model.selected_iterations == {}
    assert model.selection_info is None
    model.save(tmp_path)
    meta = json.loads((tmp_path / "lightgbm" / "meta.json").read_text())
    assert set(meta) == {"target_names", "params", "non_negative_targets", "feature_names"}
    loaded = LightGBMMultiTarget(targets)
    loaded.load(tmp_path)
    assert loaded.selection_metric == "per_target"
    assert loaded.selected_iterations == {}
    assert loaded.selection_info is None
    expected = model.predict(X[36:])
    for target, values in loaded.predict(X[36:]).items():
        np.testing.assert_array_equal(values, expected[target])
    with pytest.raises(ValueError, match="Unknown LightGBM selection metric"):
        LightGBMMultiTarget(targets, selection_metric="bogus")
