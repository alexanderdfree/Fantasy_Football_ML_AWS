"""Tests for src.shared.evaluation — compute_target_metrics, compute_ranking_metrics (TE targets)."""
...
class TestComputeRankingMetrics:
    def test_basic_structure(self, te_test_df):
        result = compute_ranking_metrics(te_test_df, "pred_total", "fantasy_points", top_k=5)
        assert "weekly" in result
        assert "season_avg_hit_rate" in result
        assert "season_avg_spearman" in result

    def test_weekly_count_matches(self, te_test_df_factory):
        df = te_test_df_factory(n_weeks=4, n_players=15)
        result = compute_ranking_metrics(df, "pred_total", "fantasy_points", top_k=12)
        assert len(result["weekly"]) == 4

    def test_hit_rate_bounds(self, te_test_df_factory):
        df = te_test_df_factory(n_weeks=2, n_players=20)
    ...
    def test_perfect_predictions_hit_rate(self, te_test_df_factory):
        df = te_test_df_factory(n_weeks=1, n_players=15)
        df["pred_total"] = df["fantasy_points"]
        result = compute_ranking_metrics(df, "pred_total", "fantasy_points", top_k=12)
        assert pytest.approx(result["weekly"][0]["top_k_hit_rate"]) == 1.0

    def test_spearman_on_perfect_prediction(self, te_test_df_factory):
        df = te_test_df_factory(n_weeks=1, n_players=20)
        df["pred_total"] = df["fantasy_points"]
        result = compute_ranking_metrics(df, "pred_total", "fantasy_points", top_k=10)
    ...
    def test_weeks_with_fewer_than_top_k_skipped(self):
        rows = [
            {"week": 1, "player_id": "TE1", "pred_total": 10, "fantasy_points": 10},
            {"week": 1, "player_id": "TE2", "pred_total": 8, "fantasy_points": 8},
        ]
    ...
    def test_single_week(self, te_test_df_factory):
        df = te_test_df_factory(n_weeks=1, n_players=15)
        result = compute_ranking_metrics(df, "pred_total", "fantasy_points", top_k=12)
