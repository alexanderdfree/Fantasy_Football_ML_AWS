...
# ---------------------------------------------------------------------------
# compute_targets — 4-head decomposition
# ---------------------------------------------------------------------------
...
class TestComputeKTargets:
    ...
    def test_does_not_mutate_original(self):
        """compute_targets returns a new frame; input columns untouched."""
        df = _make_row(fg_yards_made=60, pat_made=2, fg_missed=1, pat_missed=0)
        original_cols = set(df.columns)
        original_copy = df.copy()
        _ = compute_targets(df)
        assert set(df.columns) == original_cols, "compute_targets mutated column set"
