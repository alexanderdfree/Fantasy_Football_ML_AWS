...
class TestRidgeMultiTarget:
    ...
    def test_predict_total_matches(self, simple_data):
        ...
        preds = model.predict(X)
        np.testing.assert_allclose(total, sum(preds[t] for t in WR_TARGETS), atol=1e-6)

    def test_different_alphas(self, simple_data):
        X, y_dict = simple_data
        m1 = RidgeMultiTarget(target_names=WR_TARGETS, alpha=0.01)
        m2 = RidgeMultiTarget(target_names=WR_TARGETS, alpha=100.0)
        m1.fit(X, y_dict)
        m2.fit(X, y_dict)
    ...
    def test_save_and_load(self, simple_data, tmp_path):
        ...
        model2 = RidgeMultiTarget(target_names=WR_TARGETS, alpha=1.0)
    ...
    def test_per_target_alpha_save_load(self, simple_data, tmp_path):
        ...
        model2 = RidgeMultiTarget(target_names=WR_TARGETS)
        model2.load(model_dir)
        preds_after = model2.predict(X)
