...
@pytest.mark.unit
@pytest.mark.parametrize("source", ["ff_opportunity", "qbr_weekly", "contracts"])
@pytest.mark.parametrize("cache_error", ["bad_key_value", "empty", "corrupt_parquet"])
def test_external_malformed_cache_refetches(tmp_path, monkeypatch, source, cache_error):
    ...
    if cache_error == "corrupt_parquet":
...
@pytest.mark.unit
@pytest.mark.parametrize("source", ["ff_opportunity", "qbr_weekly", "contracts"])
@pytest.mark.parametrize("binding", ["release", "captured_dataset", "cache_only"])
def test_external_schema_empty_replay_cache_is_retained_without_source_calls(
    tmp_path, monkeypatch, source, binding
):
    ...
    loader([2024], cache_dir=str(tmp_path))
    path = tmp_path / filename
    pd.read_parquet(path).iloc[:0].to_parquet(path)
    before = path.read_bytes()
    monkeypatch.setattr(
        external_sources.nfl_source, source, lambda *a: pytest.fail("Pinned source fetched")
    )
    if binding == "release":
        monkeypatch.setenv("FF_DATA_RELEASE", "a" * 64)
    elif binding == "captured_dataset":
        monkeypatch.setenv("FF_DATASET_ID", "a" * 64)
    context = require_cached_sources(tmp_path) if binding == "cache_only" else nullcontext()
    with context:
        assert loader([2024], cache_dir=str(tmp_path)).empty
    assert path.read_bytes() == before


@pytest.mark.unit
@pytest.mark.parametrize("source", ["ff_opportunity", "qbr_weekly", "contracts"])
def test_empty_mutable_cache_cannot_be_blessed_after_failed_refresh(tmp_path, monkeypatch, source):
    from src.data.release import DataReleaseError, require_cached_sources
    ...
    monkeypatch.setattr(external_sources.nfl_source, source, unavailable)
    assert loader([2024], cache_dir=str(tmp_path)).empty
    assert not path.exists()
    with require_cached_sources(tmp_path), pytest.raises(DataReleaseError):
        loader([2024], cache_dir=str(tmp_path))
