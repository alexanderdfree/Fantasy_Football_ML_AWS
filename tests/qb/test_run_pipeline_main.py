...
``CONFIG`` directly or drives its own tiny pipeline — none touch the
...
def test_main_default_invokes_run_pipeline(monkeypatch):
    """Bare ``python run.py`` routes through ``run_pipeline``."""
...
def test_run_qb_pipeline_function_passes_through(monkeypatch):
    ...
    result = qb_pipe.run("train", "val", "test", seed=7)
    assert result == {"ok": True}
    assert seen["position"] == "QB"
    assert seen["args"][-1] == 7  # seed is positional to run_pipeline
    assert seen["cfg"] is qb_pipe.CONFIG

    # Override the cfg kwarg to confirm the fallback branch (config or CONFIG)
    custom = {"custom": True, "targets": ["x"]}
    qb_pipe.run(None, None, None, config=custom)
    assert seen["cfg"] == custom

    # Same for cv wrapper
    qb_pipe.run_cv("full", "test", seed=11)
