...
def _patch_all(monkeypatch):
    ...
    monkeypatch.setattr(dst_data, "build_data", lambda: df)
    monkeypatch.setattr(dst_targets, "compute_targets", lambda d: d)
    monkeypatch.setattr(dst_features, "compute_features", lambda d: None)
    monkeypatch.setattr(sp, "run_pipeline", _fake_pipeline)

    # Already-imported re-bound names — for direct-call tests.
    monkeypatch.setattr(dst_pipe, "build_data", lambda: df)
    monkeypatch.setattr(dst_pipe, "compute_targets", lambda d: d)
    monkeypatch.setattr(dst_pipe, "compute_features", lambda d: None)
...
def test_run_dst_pipeline_splits_by_season(monkeypatch):
    """``run`` must split the built DataFrame into train/val/test
    ...
    result = dst_pipe.run(seed=99)
    assert result["ridge_metrics"] == {}
    assert len(calls) == 1
    call = calls[0]
    assert call["position"] == "DST"
    assert call["seed"] == 99
    assert call["cfg"] is dst_pipe.CONFIG
