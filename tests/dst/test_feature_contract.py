...
* ``get_feature_columns()`` is the source of truth — it names the
  ``SPECIFIC_FEATURES + CONTEXTUAL_FEATURES`` that the NN and
  Ridge model consume.
* ``compute_features()`` is called on the FULL dataset (not per-split),
  so all rolling/EWMA/trend columns should be populated by the time the
  pipeline hands data to the model.
* ``add_specific_features`` is a no-op for DST (a legacy interface kept
...
def _build_fixture(tiny_dataset: pd.DataFrame) -> pd.DataFrame:
...
class TestDSTFeatureContract:
    ...
    def test_get_feature_columns_is_ordered_and_stable(self):
        ...
        assert cols1 == cols2
        assert cols1 == list(ALL_FEATURES)
        # No duplicates
    ...
    def test_specific_features_populated_after_compute(self, tiny_dataset):
        ...
        for col in SPECIFIC_FEATURES:
            assert col in df.columns, (
                f"compute_features did not emit '{col}' — "
                "either the feature was renamed or the computation was dropped"
            )

    def test_specific_features_are_numeric(self, tiny_dataset):
    ...
    def test_compute_drops_temp_columns(self, tiny_dataset):
        """``_dst_total_pts`` / ``_turnovers`` are scratch — must not leak."""
        df = _build_fixture(tiny_dataset)
        leaked = [c for c in df.columns if c.startswith("_")]
        assert leaked == [], f"Temp columns leaked out of compute_features: {leaked}"
    ...
    def test_raw_defensive_columns_nonnegative(self, tiny_dataset, col):
    ...
    def test_new_rolling_features_finite(self, tiny_dataset, col):
    ...
    def test_add_features_is_identity(self, tiny_dataset):
        """``add_specific_features`` is a no-op by design — assert invariant."""
        df = _build_fixture(tiny_dataset)
        train = df[df["season"] == 2022].copy()
        val = df[df["season"] == 2023].copy()
        test = df[df["season"] == 2024].copy()
        out_train, out_val, out_test = add_specific_features(train, val, test)
    ...
    def test_fill_dst_nans_produces_finite_columns(self, tiny_dataset):
        """After fill_nans, every feature column must be finite."""
        df = _build_fixture(tiny_dataset)
        ...
        # compute_features so fill_nans has something to work with.
        for split in (val, test):
            split.loc[split.index[:3], "sacks_L3"] = np.nan
            split.loc[split.index[:2], "dst_pts_L5"] = np.inf

        train, val, test = fill_nans(train, val, test, SPECIFIC_FEATURES)
        for split_name, split in [("train", train), ("val", val), ("test", test)]:
            for col in SPECIFIC_FEATURES:
                arr = split[col].astype(float).to_numpy()
                assert np.isfinite(arr).all(), (
                    f"{split_name}.{col} still has non-finite values after fill_nans"
                )
