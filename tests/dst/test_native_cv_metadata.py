...
def native_inputs(tmp_path, monkeypatch):
    ...
    scoring = extend(_make_scoring_events())
    monkeypatch.setattr(data, "load_team_week_stats", lambda seasons, cache_dir=None: stats.copy())
    # An absent versioned scoring cache would otherwise trigger a live PBP
    # load; stub it like tests/dst/test_data_build.py::synthetic_parquets.
    monkeypatch.setattr(data, "load_dst_scoring_events", lambda *a, **kw: scoring.copy())
    monkeypatch.setattr(
        data.nfl_source,
        "teams",
