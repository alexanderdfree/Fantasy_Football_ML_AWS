...
from src.dst.config import LOSS_WEIGHTS, POISSON_TARGETS, TARGETS
...
class TestMultiTargetLoss:
    ...
    def test_zero_loss_on_perfect_prediction(self):
        ...
        targets = {t: torch.tensor([5.0, 7.0]) for t in TARGETS}
    ...
    def test_weights_affect_loss(self, make_tensors):
        preds, targets = make_tensors()
        equal = {t: 1.0 for t in TARGETS}
        heavy = dict(equal)
        heavy[TARGETS[-1]] = 10.0
...
class TestMultiTargetLossPoisson:
    ...
    def test_poisson_targets_use_poisson_nll(self):
        ...
        for t in POISSON_TARGETS:
            assert isinstance(loss_fn.loss_fns[t], torch.nn.PoissonNLLLoss)
        for t in set(TARGETS) - set(POISSON_TARGETS):
    ...
    def test_poisson_component_matches_reference(self):
        ...
        preds = {t: torch.tensor([0.05, 0.12, 0.0, 0.2]) for t in TARGETS}
        targets = {t: torch.tensor([0.0, 1.0, 0.0, 2.0]) for t in TARGETS}
        _, components = loss_fn(preds, targets)
    ...
    def test_poisson_head_backward_pass(self):
        ...
        preds = {k: torch.empty(8).uniform_(0.01, 1.0).requires_grad_(True) for k in TARGETS}
        targets = {k: torch.randint(0, 3, (8,)).float() for k in TARGETS}
        combined, _ = loss_fn(preds, targets)
        combined.backward()
        for t in POISSON_TARGETS:
...
class TestMultiTargetDataset:
    ...
    def test_getitem_types(self):
        ...
        y = {
            TARGETS[0]: np.random.randn(10).astype(np.float32),
            TARGETS[1]: np.random.randn(10).astype(np.float32),
        }
        ds = MultiTargetDataset(X, y)
        x_item, y_item = ds[0]
...
class TestMultiHeadTrainer:
    ...
    def test_history_has_all_keys(self, setup_trainer):
        ...
        expected_keys.update(f"val_loss_{t}" for t in TARGETS)
        expected_keys.update(f"val_mae_{t}" for t in TARGETS)
