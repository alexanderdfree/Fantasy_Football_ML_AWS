...
def test_reference_ranks_shared_components_while_display_preserves_full_forecasts():
    frame = pd.DataFrame(
        [
            {
                "player_id": "rush",
    ...
    assert project_nflcom_to_fantasy(frame, "WR").iloc[0].nflcom_pred_total == 11
    assert reference.iloc[0].player_id == "receive"
    assert reference.iloc[0].reference_pred == 6
...
def test_full_forecast_and_shared_comparison_totals_are_distinct():
    frame = _normalize_one_position(deebo_raw(), "WR").assign(player_id="00-0035719")
    assert project_nflcom_to_fantasy(frame, "WR").iloc[0].nflcom_pred_total == pytest.approx(15.625)
    assert project_expert_comparison(frame, "WR", source="nflcom").iloc[
        0
    ].expert_pred_total == pytest.approx(13.174)
    missing = frame.drop(columns="fumbles_lost")
    assert np.isnan(project_expert_comparison(missing, "WR").iloc[0].expert_pred_total)
