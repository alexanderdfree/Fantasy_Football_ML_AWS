...
def _launch_ab(tmp_path, only, argv_stub=None):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("baseline", ["baseline"]),
        ("+season_recency", ["baseline", "+season_recency"]),
        ("baseline +season_recency", ["baseline", "+season_recency"]),
    ],
)
def test_ab_workflow_forwards_variant_names_as_argv_data(tmp_path, only, expected):
    result, args_file = _launch_ab(tmp_path, only)
    assert result.returncode == 0, result.stdout + result.stderr
    argv = json.loads(args_file.read_text())
    assert argv[:2] == ["-m", "src.tuning.launch_ab"]
    args = _build_parser().parse_args(argv[2:])
    assert args.spec == "src.tuning.ab_example"
    assert args.image_sha == "a" * 40
    assert args.positions == ["RB"]
    assert args.seeds == [42]
    assert list(resolve_spec(args.spec, only=args.only).variants) == expected


@pytest.mark.parametrize("only", ["$(touch${IFS}injected)", "baseline;touch injected", "*", "../x"])
def test_ab_workflow_rejects_non_canonical_variant_names_before_launch(tmp_path, only):
    """Rejected names never reach the launcher; ``*`` is a control for ``set -f``
    (without it, word splitting would expand the pattern to files in the cwd,
    which pass the name check)."""
    result, args_file = _launch_ab(tmp_path, only)
    assert result.returncode != 0
    assert "Invalid variant name" in result.stdout + result.stderr
