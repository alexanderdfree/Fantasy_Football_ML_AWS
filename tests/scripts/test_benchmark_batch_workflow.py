...
_RESOLVE_STEP = "Resolve image SHA + experiment prefix (validate dispatch inputs)"
...
def checkout(tmp_path):
    """A throwaway git checkout whose HEAD stands in for the dispatched ref.

    The Resolve step expands an abbreviated ``image_sha`` with ``git rev-parse``
    against the runner's fetch-depth-0 checkout, so accepted inputs must name a
    real commit.
    """
...
@pytest.mark.parametrize("form", ["blank", "short", "upper", "full"])
def test_resolve_image_accepts_documented_sha_tags(checkout, form):
    repo, head = checkout
    sha = {"blank": "", "short": head[:7], "upper": head[:7].upper(), "full": head}[form]
    result = _run(
        _step(_RESOLVE_STEP), repo, INPUT_SHA=sha, GITHUB_SHA=head, POSITIONS="QB RB", SEED="42"
    )
    assert result.returncode == 0, result.stderr
    assert (repo / "outputs").read_text().splitlines() == [
        f"sha={head}",
        f"prefix=experiments/benchmark/{head}/models",
    ]
...
def test_resolve_image_rejects_invalid_input_before_publishing_outputs(tmp_path, sha):
    result = _run(_step(_RESOLVE_STEP), tmp_path, INPUT_SHA=sha, POSITIONS="QB", SEED="42")
...
def test_resolve_image_rejects_well_formed_sha_missing_from_checkout(checkout):
    repo, _ = checkout
    result = _run(_step(_RESOLVE_STEP), repo, INPUT_SHA="b" * 40, POSITIONS="QB", SEED="42")
    assert result.returncode != 0
