...
def test_study_storage_is_versioned_for_scheduler_search_space():
    assert tune_nn._study_name("RB") == "nn_scheduler_v3_fp_rmse_ppr_v1_rb"
    assert tune_nn._study_db_path("RB") == "tune_nn_scheduler_v3_fp_rmse_ppr_v1_rb.db"
    assert tune_nn._s3_key_prefix("RB") == "tune_nn/scheduler_v3_fp_rmse_ppr_v1/rb"
...
def test_mps_graph_storage_profile_is_separate_from_eager():
    ...
    assert version == "scheduler_v3_fp_rmse_ppr_v1_mps_graph"
    assert tune_nn._study_name("RB", version) == "nn_scheduler_v3_fp_rmse_ppr_v1_mps_graph_rb"
    assert (
        tune_nn._study_db_path("RB", version)
        == "tune_nn_scheduler_v3_fp_rmse_ppr_v1_mps_graph_rb.db"
    )
    assert (
        tune_nn._s3_key_prefix("RB", version) == "tune_nn/scheduler_v3_fp_rmse_ppr_v1_mps_graph/rb"
    )
...
def test_resolve_storage_version_follows_live_capture_decision(monkeypatch):
    ...
    assert tune_nn._resolve_storage_version("mps") == (
        "scheduler_v3_fp_rmse_ppr_v1_mps_graph",
        True,
        False,
    )
    ...
    assert tune_nn._resolve_storage_version("thread") == (
        "scheduler_v3_fp_rmse_ppr_v1_graphfull",
        True,
        True,
    )
    assert tune_nn._resolve_storage_version("mps") == (
        "scheduler_v3_fp_rmse_ppr_v1_mps_graphfull",
        True,
        True,
    )
...
def test_resolve_storage_version_ignores_env_when_capture_disagrees(monkeypatch):
    ...
    monkeypatch.setattr(tune_nn, "_cuda_graph_enabled", lambda: False)
    monkeypatch.setattr(tune_nn, "_cuda_graph_full_enabled", lambda: False)
    assert tune_nn._resolve_storage_version("mps")[0] == "scheduler_v3_fp_rmse_ppr_v1_mps"
    # sm_80+ box + env unset: autodetect captures -> graph namespace.
    monkeypatch.delenv("FF_CUDA_GRAPH", raising=False)
    monkeypatch.setattr(tune_nn, "_cuda_graph_enabled", lambda: True)
    monkeypatch.setattr(tune_nn, "_cuda_graph_full_enabled", lambda: False)
    assert tune_nn._resolve_storage_version("thread")[0] == "scheduler_v3_fp_rmse_ppr_v1_graph"
...
def test_graphfull_storage_profiles_compose_only_with_graph():
    ...
    assert (
        rs("mps", cuda_graph=True, full_graph=True) == "scheduler_v3_fp_rmse_ppr_v1_mps_graphfull"
    )
    assert rs("thread", cuda_graph=True, full_graph=True) == "scheduler_v3_fp_rmse_ppr_v1_graphfull"
    assert rs("mps", cuda_graph=False, full_graph=True) == "scheduler_v3_fp_rmse_ppr_v1_mps"
    assert rs("thread", cuda_graph=False, full_graph=True) == "scheduler_v3_fp_rmse_ppr_v1"
    assert rs("mps", cuda_graph=True, full_graph=False) == "scheduler_v3_fp_rmse_ppr_v1_mps_graph"
