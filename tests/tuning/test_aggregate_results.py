...
def test_download_skips_404(tmp_path, capsys):
    ...
    with patch("boto3.client", return_value=fake_s3):
        paths = aggregate_results._download_from_s3(
            "bucket", ["QB", "RB"], str(tmp_path), "scheduler_v3_fp_rmse_ppr_v1"
        )
    assert len(paths) == 1
    assert "rb" in paths[0]
    assert calls == [
        "tune_nn/scheduler_v3_fp_rmse_ppr_v1/qb/results.json",
        "tune_nn/scheduler_v3_fp_rmse_ppr_v1/rb/results.json",
    ]
    out = capsys.readouterr().out
...
def test_download_from_s3_uses_requested_search_space_version(tmp_path):
    ...
    with patch("boto3.client", return_value=fake_s3):
        aggregate_results._download_from_s3(
            "bucket", ["QB"], str(tmp_path), "scheduler_v3_fp_rmse_ppr_v1_mps_graph"
        )

    assert calls == ["tune_nn/scheduler_v3_fp_rmse_ppr_v1_mps_graph/qb/results.json"]
