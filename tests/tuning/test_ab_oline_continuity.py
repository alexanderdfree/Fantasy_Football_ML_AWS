...
def _synthetic_result(with_elite_col=True):
    ...
    elite = (
        {
            "status": "available",
            "models": {"Ridge": {"n": 12, "mae": 2.5, "bias": 1.25}},
        }
        if with_elite_col
        else {"status": "unavailable", "models": {}}
    )
    return {"test_df": df, "cohorts": {"elite_top24": elite}}
...
def test_metric_fn_uses_canonical_cohort_without_legacy_column_fallback():
    with_elite = spec.metric_fn(_synthetic_result(True), "RB")
    assert with_elite["Ridge"]["elite24_bias"] == 1.25
    assert with_elite["Ridge"]["elite24_mae"] == 2.5
    assert with_elite["Ridge"]["elite24_n"] == 12
    without = spec.metric_fn(_synthetic_result(False), "RB")
    assert "elite24_mae" not in without["Ridge"]
    assert without["Ridge"]["elite24_available"] == 0
