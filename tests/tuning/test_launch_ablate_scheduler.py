...
def test_scheduler_launcher_requires_complete_success(remote, monkeypatch, wait, case):
    ...
    def submit(position, **kwargs):
        ...
        # The resolved binding also carries image URIs; this launcher only
        # consumes the source SHA and the GPU/CPU job definitions.
        assert kwargs["binding"]["image_sha"] == SHA_A
        assert kwargs["binding"]["gpu_definition"] == "gpu:7"
        assert kwargs["binding"]["cpu_definition"] == ""
