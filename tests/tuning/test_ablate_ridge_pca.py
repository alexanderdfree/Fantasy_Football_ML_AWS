...
def test_execute_job_keeps_orchestration_metadata_out_of_typed_recipe(
    ...
    def typed_runner(*, train_df, val_df, test_df, seed, config):
        ...
        assert train_df is frames["train"] and seed == 42
        if not calls:
            # Val-as-test scores validation once: Ridge fits on train only.
            assert val_df.empty and list(val_df.columns) == list(frames["val"].columns)
        else:
            assert val_df is frames["val"]
...
def test_ridge_job_uses_native_frames_and_preserves_holdouts_and_seed(monkeypatch, position):
    ...
    import src.shared.position_data as position_data
