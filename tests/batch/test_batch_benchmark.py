...
@pytest.fixture()
def _identified_training(monkeypatch):
    from src.batch import launch
    ...
    monkeypatch.setenv("FF_LEGACY_RUN_ID", "unit-benchmark-request")
    monkeypatch.setenv("FF_TRAIN_GIT_SHA", "abc1234" + "0" * 33)
    from src.batch import launch, run_history

    monkeypatch.setattr(launch, "JOB_DEFINITION_REVISION", "1")
    monkeypatch.setattr(launch, "register_submission_source", lambda *_: None)
    monkeypatch.setattr(run_history, "create_run", lambda *_, **__: "unit-history-run")


@pytest.fixture()
def _main_stubs(tmp_path, monkeypatch, _identified_training):
    ...
    def _submit_job(pos, seed, batch_client=None, *, history_run_id=None, binding=None):
    ...
    def _upload_data(bucket, **kwargs):
        launched.append({"upload": bucket})

    monkeypatch.setattr(bb, "submit_job", _submit_job)
    monkeypatch.setattr(launch, "upload_data", _upload_data)
    ...
    monkeypatch.setattr(
        ...
        "complete_run",
        lambda *_, **__: {
            "run_id": "unit-history-run",
            "timestamp": "2026-09-10T00:00:00",
            "git_hash": "abc1234",
            "note": "unit test",
            "backend": "batch",
            "positions": ["QB", "RB"],
            "results": list(fake_metrics.values()),
        },
    )
...
@pytest.mark.unit
def test_main_empty_metrics_early_returns(monkeypatch, tmp_path, _identified_training):
    ...
    monkeypatch.setattr(bb, "submit_job", lambda pos, seed, **kw: (pos, "id"))
    ...
    monkeypatch.setattr(
        "sys.argv", ["src/batch/benchmark.py", "--positions", "QB", "--download-only"]
    )
    bb.main()
...
@pytest.mark.unit
def test_main_reports_failed_jobs(monkeypatch, tmp_path, capsys, _identified_training):
...
@pytest.mark.unit
def test_main_reports_submit_exception(monkeypatch, tmp_path, capsys, _identified_training):
...
def test_record_benchmark_run_fingerprints_only_sha_matched_positions(_main_stubs, monkeypatch):
    ...
    with pytest.raises(ValueError, match="mismatched training history"):
