"""Tests for shared.training — MultiTargetLoss, dataloaders, trainer (Kicker targets)."""
...
class TestMultiTargetLoss:
    ...
    def test_component_keys(self):
        ...
        assert set(components.keys()) == {
            "loss_fg_points", "loss_pat_points",
...
class TestMultiTargetDataset:
    ...
    def test_single_element(self):
        ...
        y = {"pat_points": np.array([3.0], dtype=np.float32)}
        ds = MultiTargetDataset(X, y)
        x_item, y_item = ds[0]
        assert pytest.approx(x_item[0].item()) == 1.0
        assert pytest.approx(y_item["pat_points"].item()) == 3.0
...
class TestMultiHeadTrainer:
    ...
    def setup_trainer(self):
        ...
        y_train = {t: np.random.randn(n_train).astype(np.float32) for t in K_TARGETS}
        y_val = {t: np.random.randn(n_val).astype(np.float32) for t in K_TARGETS}

        y_train["total"] = sum(y_train[t] for t in K_TARGETS)
        y_val["total"] = sum(y_val[t] for t in K_TARGETS)
        ...
        model = MultiHeadNet(
            input_dim=d, target_names=K_TARGETS,
            backbone_layers=[16, 8], head_hidden=4, dropout=0.1,
        )
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=3, factor=0.5)
        criterion = MultiTargetLoss(target_names=K_TARGETS, loss_weights=K_LOSS_WEIGHTS)
        device = torch.device("cpu")

        trainer = MultiHeadTrainer(
            model, optimizer, scheduler, criterion, device,
            target_names=K_TARGETS, patience=5,
        )
    ...
    def test_history_has_all_keys(self, setup_trainer):
        ...
        expected_keys = {
            "train_loss", "val_loss",
            "val_loss_fg_points", "val_loss_pat_points",
            "val_mae_total", "val_mae_fg_points", "val_mae_pat_points",
            "val_rmse_total",
        }
    ...
    def test_early_stopping(self):
        ...
        y_train = {t: np.random.randn(n_train).astype(np.float32) for t in K_TARGETS}
        y_train["total"] = sum(y_train[t] for t in K_TARGETS)
        y_val = {t: np.random.randn(n_val).astype(np.float32) * 10 for t in K_TARGETS}
        y_val["total"] = sum(y_val[t] for t in K_TARGETS)
        ...
        trainer = MultiHeadTrainer(
            ...
            torch.device("cpu"), target_names=K_TARGETS, patience=3,
        )
