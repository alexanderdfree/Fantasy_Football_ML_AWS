...
Kickers have only 2 targets (fg_points, pat_points).
"""
...
def simple_data():
    ...
    y_dict = {
        "fg_points": np.random.rand(n).astype(np.float32) * 10,
...
class TestRidgeMultiTarget:
    ...
    def test_total_is_sum_of_components(self, simple_data):
        ...
        model.fit(X, y_dict)
        preds = model.predict(X)
        expected = preds["fg_points"] + preds["pat_points"]
    ...
    def test_feature_importance_keys(self, simple_data):
        ...
        assert set(importance.keys()) == {"fg_points", "pat_points"}
    ...
    def test_constant_target(self):
        ...
        y_dict = {
            "fg_points": np.full(10, 7.0),
            "pat_points": np.full(10, 2.0),
        }
        model = RidgeMultiTarget(target_names=K_TARGETS, alpha=1.0)
        model.fit(X, y_dict)
        preds = model.predict(X)
        assert np.allclose(preds["fg_points"], 7.0, atol=1.0)

    def test_per_target_alpha_construction(self, simple_data):
        X, y_dict = simple_data
        alpha_dict = {"fg_points": 0.01, "pat_points": 100.0}
        ...
        any_different = any(
            not np.allclose(p_per[t], p_uniform[t]) for t in K_TARGETS
        )
        assert any_different

    def test_per_target_alpha_save_load(self, simple_data, tmp_path):
        X, y_dict = simple_data
        alpha_dict = {"fg_points": 0.1, "pat_points": 500.0}
    ...
    def test_alpha_dict_missing_key_raises(self):
        """Kicker has only 2 targets — if only one is in alpha dict, error."""
