"""TE-specific evaluation utilities."""
...
def compute_te_metrics(y_true_dict: dict, y_pred_dict: dict) -> dict:
    """Compute per-target and total metrics for TE model.

    Args:
        y_true_dict: {"receiving_floor": ..., "rushing_floor": ..., "td_points": ..., "total": ...}
    ...
    """
    results = {}
    for target in ["total", "receiving_floor", "rushing_floor", "td_points"]:
        results[target] = compute_metrics(y_true_dict[target], y_pred_dict[target])
    return results


def compute_te_ranking_metrics(
    test_df: pd.DataFrame,
    pred_col: str = "pred_total",
    true_col: str = "fantasy_points",
    top_k: int = 12,
) -> dict:
    """Per-week ranking quality metrics for TE model.
...
def print_te_comparison_table(results: dict) -> None:
    """Pretty-print comparison of all TE models."""
    print("\n" + "=" * 80)
    print("TE Model Comparison -- Total Fantasy Points")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TE Model Comparison -- Per-Target MAE")
    print("=" * 80)
    print(f"{'Model':<30} {'Recv Floor':>12} {'Rush Floor':>12} {'TD Pts':>12}")
    print("-" * 68)
    for model_name, metrics in results.items():
        if "receiving_floor" in metrics:
            print(
                f"{model_name:<30} "
...
def plot_te_pred_vs_actual(
    ...
    targets = [("total", "Total Fantasy Points"), ("receiving_floor", "Receiving Floor"),
