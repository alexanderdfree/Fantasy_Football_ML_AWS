"""TE-specific multi-head neural network for fantasy point decomposition."""
...
class TEMultiHeadNet(nn.Module):
    """Multi-head neural network for TE fantasy point decomposition.
    ...
    The td_head supports a separate hidden size (td_head_hidden) because
    TE TD scoring is boom/bust and benefits from a larger head.
    """
    ...
    def __init__(
        ...
        head_hidden: int = 24,
        td_head_hidden: int = None,
        dropout: float = 0.35,
    ):
        super().__init__()
        if backbone_layers is None:
            backbone_layers = [96, 48]
        ...
        # TD head can have a different hidden size (boom/bust TE scoring)
        td_h = td_head_hidden if td_head_hidden is not None else head_hidden
        self.td_head = nn.Sequential(
            nn.Linear(backbone_out_dim, td_h),
            nn.ReLU(),
            nn.Linear(td_h, 1),
        )
