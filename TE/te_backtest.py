"""TE-specific weekly backtest simulation."""
...
def run_te_weekly_simulation(
    test_df: pd.DataFrame,
    pred_columns: dict,
    true_col: str = "fantasy_points",
    top_k: int = 12,
) -> dict:
    """Week-by-week simulation for TE models across the test season.

    Args:
        test_df: TE-only test DataFrame
...
def plot_te_weekly_accuracy(sim_results: dict, save_path: str) -> None:
    """Two-panel figure: weekly MAE and top-12 hit rate for TE models."""
    ...
    ax1.set_xlabel("Week")
    ax1.set_ylabel("MAE")
    ax1.set_title("TE Weekly MAE by Model")
    ax1.legend()
    ...
    ax2.set_title("TE Weekly Top-12 Hit Rate")
    ax2.legend()
