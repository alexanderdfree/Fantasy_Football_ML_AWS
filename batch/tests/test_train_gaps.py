...
def test_validate_remote_tarball_raises_when_metrics_missing():
    from batch.train import _validate_remote_tarball
...
def test_run_rb_gate_ablation_runs_three_variants(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "RB.run_rb_pipeline.run_rb_pipeline", _canned_run_rb_pipeline, raising=False
    )
...
def test_run_rb_gate_ablation_drop_gate_when_margins_are_tiny(monkeypatch, capsys):
    ...
    monkeypatch.setattr("RB.run_rb_pipeline.run_rb_pipeline", _canned, raising=False)
    import pandas as pd
...
def test_run_rb_gate_ablation_raises_if_attn_metrics_missing(monkeypatch):
    ...
    monkeypatch.setattr(
        "RB.run_rb_pipeline.run_rb_pipeline",
