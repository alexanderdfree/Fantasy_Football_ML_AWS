...
| `task-definition.json` | ECS task def template (ARM64 Fargate, 2 vCPU / 8 GB, `/ready` check, artifact-only runtime). Placeholders `__ACCOUNT_ID__`, `__REGION__`, `__ECR_URI__`, `__IMAGE_TAG__`, `__FF_MODEL_S3_BUCKET__` are substituted by `bootstrap.sh`. |
| `task-role-policy.json` | Read model/serving generations and benchmark history; scoped listing and prior-version recovery for the upcoming artifact |
...
Prior-version recovery requires S3 versioning, `GetObjectVersion` for only
`models/predictions_cache/upcoming_week.json`, and `ListBucketVersions` with that
exact prefix. Bootstrap reconciles these statements from `task-role-policy.json`.
Deploy does not reconcile IAM: apply those statements before rolling out recovery
(or use bootstrap for full infrastructure reconciliation). The task remains a
read-only consumer and cannot publish prediction caches.
