...
- **Install:** per-platform torch wheels exist — [requirements-dev.txt](requirements-dev.txt) (cpu), [requirements-gpu.txt](requirements-gpu.txt) (cu128 / Blackwell sm_120), [src/batch/Dockerfile.train](src/batch/Dockerfile.train) (cu126 / T4·L4). Extend these; no ad-hoc pins.
...
- **GPU arch is per-CUDA-wheel, not per-version:** sm_xx comes from the cuXXX wheel build (`2.11.0+cu128` adds Blackwell sm_120) — verify the wheel index, don't trust a version→GPU claim.
