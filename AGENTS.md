...
This repo is regularly worked from agent worktrees (`.claude/worktrees/<name>` or `~/.codex/worktrees/<id>/Final-Project`) where the parent worktree holds `main`. Three quirks apply to any agent:
...
| Post-PR: rebase → review → apply nits → merge on green | `post-pr-create.sh` + `/review` | [`.codex/hooks/post-pr-create.sh`](.codex/hooks/post-pr-create.sh) injects the workflow; review equivalent is `codex review --base origin/main` |
...
Codex local setup: run [scripts/bootstrap-codex-local.sh](scripts/bootstrap-codex-local.sh) to install/update the user-home prompt copies, restart Codex, then review/trust the project hooks with `/hooks`. Custom prompts are user-home scoped (`$CODEX_HOME/prompts`), so the tracked [`.codex/prompts/`](.codex/prompts/) files are templates, not directly loaded by Codex.
