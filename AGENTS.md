...
| Post-merge: fast-forward the parent/main checkout's `main` to `origin/main` (guarded ff-only; skips a dirty / non-`main` parent) | `post-pr-merge.sh` (PostToolUse on `gh pr merge`) | [`.codex/hooks/post-pr-merge.sh`](.codex/hooks/post-pr-merge.sh) — same parser-aware `gh pr merge` matcher + `*_refresh_parent_main` helper, mirrored in `.codex/hooks/lib.sh` |
