...
This repo is regularly worked from `.claude/worktrees/<name>` clones where the parent worktree holds `main`. Three quirks apply to any agent:

- **Edit files in the worktree, not the parent checkout.** Plan files and search tools often report repo-relative or *parent*-absolute paths (`/…/Final-Project/src/foo.py`). Writing those verbatim silently edits the parent (`main`'s checkout) instead of this feature branch — `git status` in the worktree stays clean and any benchmark re-run uses the *unchanged* code (MAE Δ=0.0000 on every target is the late smell). Re-prefix to `/…/Final-Project/.claude/worktrees/<name>/…`, then `grep` the new symbol in the worktree file to confirm the edit landed. (Claude Code has a deterministic guard hook for this — see `CLAUDE.md`.)
...
- **Small FP-MAE deltas need ≥8 seeds, reported mean±std.** 3 seeds can fake a borderline result (backbone-norm: 3-seed −0.022±0.019 → 8-seed +0.007±0.034 = noise). Local runs ~1.5 min/variant.
...
The Claude-Code machinery (hooks, skills, the scheduled audit routine, sub-agent contracts) lives in `CLAUDE.md`; this section is the **Codex-side parity stub** — fill it in as the matching Codex hooks/skills are built so Codex follows the same discipline the hooks enforce for Claude. Until each row is wired, **apply the behaviour manually**.

| Discipline (what it enforces) | Claude Code mechanism | Codex equivalent |
|---|---|---|
| Auto-format Python after an edit | `ruff-format.sh` (PostToolUse) | _(to fill)_ |
| Pre-PR gate: `ruff check` + `ruff format --check` + `pytest -m unit` + benchmark-freshness for pipeline edits | `pre-pr.sh` (PreToolUse on `gh pr create`) | _(to fill)_ |
| Block edits that target the parent checkout from inside a worktree | `guard-worktree-path.sh` (PreToolUse) | _(to fill)_ |
| Env bootstrap (venv + deps + `PYTHONPATH`) at session start | `session-start.sh` (SessionStart) | _(to fill)_ |
| Post-PR: rebase → review → apply nits → merge on green | `post-pr-create.sh` + `/review` | _(to fill)_ |
