...
- **Evaluation cohorts (ADR-0024):** compare identical regular-season player-weeks
  ...
  `elite_top24`); `weekly_reference_top24` is secondary. A winner needs the served
  model's paired CI (`SERVED_MODEL`, not best-of-four) to exclude zero on MAE and
  RMSE; hindsight cohorts carry no verdict. NFL.com offense is not graded (a stale
