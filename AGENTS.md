...
| Triage the severity + model regress-risk labeled `claude-audit` / `codex-audit` issue backlog into tier-by-risk PRs | `solve-issues` skill | `/prompts:solve-issues` from [`.codex/prompts/solve-issues.md`](.codex/prompts/solve-issues.md) after `scripts/bootstrap-codex-local.sh` |
