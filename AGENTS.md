...
| Post-PR: rebase → review → apply nits → merge on green | `post-pr-create.sh` + `/review` | [`.codex/hooks/post-pr-create.sh`](.codex/hooks/post-pr-create.sh) emits a compact pointer to the prompt-backed workflow; review equivalent is `codex review --base origin/main` |
