"""WR-specific training infrastructure: loss, dataset, dataloaders, and trainer."""
...
class MultiTargetLoss(nn.Module):
    """Combined loss for multi-head WR network.
...
class WRMultiTargetDataset(Dataset):
...
def make_wr_dataloaders(X_train, y_train_dict, X_val, y_val_dict, batch_size=256):
    """Create DataLoaders for multi-target WR training."""
    train_ds = WRMultiTargetDataset(X_train, y_train_dict)
    val_ds = WRMultiTargetDataset(X_val, y_val_dict)
...
class WRMultiHeadTrainer:
    """Training loop for multi-head WR network."""
    ...
    def plot_training_curves(self, history: dict, save_path: str) -> None:
        """Multi-panel figure for WR multi-head training."""
