"""Tests for shared.training — MultiTargetLoss, dataloaders, trainer (WR targets)."""
...
WR_LOSS_WEIGHTS = {"receiving_floor": 1.0, "rushing_floor": 1.0, "td_points": 1.0}


class TestMultiTargetLoss:
    def _make_tensors(self, n=10):
        preds = {t: torch.randn(n) for t in WR_TARGETS}
        preds["total"] = torch.randn(n)
        targets = {t: torch.randn(n) for t in WR_TARGETS}
    ...
    def test_zero_loss_on_perfect_prediction(self):
        loss_fn = MultiTargetLoss(target_names=WR_TARGETS, loss_weights=WR_LOSS_WEIGHTS)
        targets = {
            "receiving_floor": torch.tensor([10.0, 12.0]),
            "rushing_floor": torch.tensor([0.5, 1.0]),
            "td_points": torch.tensor([6.0, 0.0]),
            "total": torch.tensor([16.5, 13.0]),
        }
    ...
    def test_weights_affect_loss(self):
        ...
        loss_recv_heavy = MultiTargetLoss(
            target_names=WR_TARGETS,
            loss_weights={"receiving_floor": 10.0, "rushing_floor": 1.0, "td_points": 1.0},
            w_total=1.0,
        )
        c1, _ = loss_equal(preds, targets)
        c2, _ = loss_recv_heavy(preds, targets)
    ...
    def test_backward_pass(self):
        loss_fn = MultiTargetLoss(target_names=WR_TARGETS, loss_weights=WR_LOSS_WEIGHTS)
        preds = {k: torch.randn(5, requires_grad=True) for k in WR_TARGETS + ["total"]}
...
class TestMultiHeadTrainer:
    ...
    def setup_trainer(self):
        ...
        model = MultiHeadNet(
            input_dim=d, target_names=WR_TARGETS,
        ...
        trainer = MultiHeadTrainer(
            ...
            target_names=WR_TARGETS, patience=5,
        )
    ...
    def test_early_stopping(self):
        ...
        trainer = MultiHeadTrainer(
            ...
            torch.device("cpu"), target_names=WR_TARGETS, patience=3,
        )
