...
Canonical list: [requirements.txt](requirements.txt) (serving), [batch/requirements.txt](batch/requirements.txt) (training), and [requirements-dev.txt](requirements-dev.txt) (torch + tooling). Key choices:
...
- **Softplus floor inflating low-scoring predictions** — AI-generated NN used `softplus` on head outputs, which has a floor of ~0.69. Across three heads this imposed a ~2-point minimum no player could drop below. Alex identified this by noticing the prediction distribution was off, traced it to the activation, and swapped to `torch.clamp(min=0)`. See [shared/neural_net.py](shared/neural_net.py) (head-output gating, around line 100) and the first entry in the TODO archive.
- **DST `pts_allowed_bonus` range bug** — the clamp fix above then incorrectly clamped DST's `pts_allowed_bonus` head, which legitimately ranges from −4 to +10. Alex added a per-position `non_negative_targets` parameter so only the heads that *should* be non-negative are constrained. The CV path was later caught missing the same kwarg and fixed ([shared/pipeline.py](shared/pipeline.py)).
