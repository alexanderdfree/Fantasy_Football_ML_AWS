...
- **Vegas odds + weather features** — joined in [src/shared/weather_features.py](src/shared/weather_features.py). Source and feature rationale are documented in [docs/design_weather_and_odds.md](docs/design_weather_and_odds.md).
...
- **Weather/Vegas features missing at inference** — training merged schedule features but serving did not. Models trained on 12 weather/Vegas features received zeros at serving time. Fix in [src/serving/app.py](src/serving/app.py) (`merge_schedule_features` call inside `_apply_position_models`, around line 351).
