...
pregame reference (the served Comparison tab's headline is the symmetric `weekly_consensus_top24`). Cohort scores use only shared projected components on both predictions and actuals (`actual_basis=shared_projected_components_v2`); a provider row with every published stat at zero is a missing forecast. The reference recipe `shared_components_v4` uses NFL.com/RotoWire for offense, ESPN for K, and RotoWire for DST. Missing reference/prior/component data is reported explicitly.
