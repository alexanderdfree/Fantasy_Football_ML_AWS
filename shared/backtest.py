...
def run_weekly_simulation(test_df, pred_columns, true_col="fantasy_points", top_k=12):
        ...
        for model_name, pred_col in pred_columns.items():
            ...
            if len(week_df) >= top_k:
                ...
                weekly_ranking[model_name].append({
                    "week": week, "top_k_hit_rate": hit_rate, "spearman": corr,
                })

    season_summary = {}
    for name in pred_columns:
        if season_preds[name]:
            season_summary[name] = compute_metrics(
                np.array(season_true), np.array(season_preds[name])
            )
        else:
            season_summary[name] = {"mae": float("nan"), "rmse": float("nan"), "r2": float("nan")}
...
def plot_weekly_accuracy(sim_results, save_path, pos_label=""):
    ...
    ax1.set_xlabel("Week")
    ax1.set_ylabel("MAE")
    ax1.set_title(f"{pos_label} Weekly MAE by Model")
    ax1.legend()
    ...
    ax2.set_xlabel("Week")
    ax2.set_ylabel(f"Top-{12} Hit Rate")
    ax2.set_title(f"{pos_label} Weekly Top-{12} Hit Rate")
    ax2.legend()
