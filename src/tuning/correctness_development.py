"""Paired numerical correctness checks using the existing historical Batch harness.

All fits use production settings, immutable inputs, and restored-weight scoring.
Metric regressions are reported; correctness and compatibility are merge gates.
"""
...
BASELINE_SHA = "a69a813df42e10f655c1f0b9105365c48a2d64cb"
...
def select_changes(arm):
    """Run the frozen main likelihood or the exact PR implementation per arm."""
    ...
    for name in ("negbin2_log_prob", "ztnb2_log_prob", "ztp_log_prob"):
        ...
        candidate = getattr(current, "_correctness_candidate", current)
        original = getattr(correctness_legacy_count, name)
        original._correctness_candidate = candidate
        setattr(training, name, candidate if arm in {"precision", "combined"} else original)


def configured_position(config):
    """WR and TE share targets; native provider identity is unambiguous."""
    module = getattr(config.get("filter_fn"), "__module__", "")
    expected = {f"src.{p.lower()}.data": p for p in ("QB", "RB", "WR", "TE", "K", "DST")}
...
def install_observer():
    ...
    def train(trainer, train_loader, val_loader, n_epochs):
        ...
        from src.analysis.correctness_count_diagnostics import (
            error_summary,
...
def configure(config, *, arm):
    ...
    origin = int(os.environ.get("FF_CORRECTNESS_ORIGIN", "2022"))
    ...
    select_changes(arm)
    config = dict(config, nn_correct_ztnb_mean=arm in {"mean", "combined"})
    install_observer()
...
def variants(*, native=False, controls=False):
    ...
    return [
        ...
        for arm in (("baseline", "combined") if controls else ARMS)
    ]
...
def metric_fn(result, position):
    ...
    metadata = {
        ...
        "gate": "correctness_and_compatibility; metric_changes_reported",
        "probability_mean": "corrected" if STATE["arm"] in {"mean", "combined"} else "legacy",
        "position": position,
        "origin": STATE["origin"],
        "variant": STATE["arm"],
        "mode": "paired_historical_correctness",
