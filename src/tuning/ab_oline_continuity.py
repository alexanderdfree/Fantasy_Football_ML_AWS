...
def metric_fn(result: dict, position: str) -> dict[str, dict[str, float]]:
    ...
    elite = result.get("cohorts", {}).get("elite_top24", {})
    ...
    for name in models:
        ...
        cell = elite.get("models", {}).get(name, {})
        available = elite.get("status") == "available" and bool(cell.get("n"))
        row["elite24_available"] = float(available)
        row["elite24_n"] = float(cell.get("n", 0) or 0)
        if available:
            row["elite24_mae"] = float(cell["mae"])
            row["elite24_bias"] = float(cell["bias"])
        out[name] = row
    return out
