...
def train_stacked(
    ...
    if epoch_callback is not None and trainer0.selection_metric != "fantasy_rmse_ppr":
        # The per-epoch report is PPR fantasy RMSE (``stacked_val_rmse``); the
        # captured trainers must select on the same quantity. tune_nn sets
        # this itself per trial — see ``_make_stacked_objective``.
        raise ValueError(
            "Stacked tuning reports validation PPR fantasy RMSE per epoch; capture "
            "the seeds with cfg['nn_selection_metric'] = 'fantasy_rmse_ppr' "
            f"(got {trainer0.selection_metric!r})"
        )
