...
class LightGBMMultiTarget:
    ...
    def save(self, model_dir):
        ...
        if self.selected_iterations:
            # Joint-prefix artifacts only: a per-target (legacy) fit keeps the
            # exact meta layout main wrote, and ``load`` defaults the absent
            # keys back to the per-target contract.
            meta["selection_metric"] = self.selection_metric
            meta["selected_iterations"] = self.selected_iterations
            meta["selection_info"] = self.selection_info
