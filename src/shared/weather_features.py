...
def build_implied_team_total_lookup(schedules: pd.DataFrame) -> pd.DataFrame:
    ...
    nflverse ``spread_line`` is positive when the home team is favored, so
    the home team's implied total is ``(total_line + spread_line) / 2`` and
    the away team's is ``(total_line - spread_line) / 2``. The enriched
    ``_build_team_schedule_lookup`` used by ``merge_schedule_features`` applies
    the same convention. ``src.features.engineer._build_defense_matchup_features``
    consumes this minimal lookup on the direct ``build_features`` path.
