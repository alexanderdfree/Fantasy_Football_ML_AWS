...
from src.data import forecast_weather, nfl_source
from src.data.dst_scoring import PBP_COLUMNS, aggregate_dst_scoring_events, load_dst_scoring_events
from src.data.live_build import historical_cache_dir
...
def fetch_live_inputs(season: int, week: int, historical_weekly, historical_team) -> LiveInputs:
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        weekly_job = pool.submit(copy_context().run, nfl_source.weekly_data, completed_seasons)
        team_jobs = [
            pool.submit(copy_context().run, nfl_source.team_week_stats_release, s)
            for s in completed_seasons
        ]
        pbp_job = pool.submit(
            copy_context().run, nfl_source.pbp_data, completed_seasons, _LIVE_PBP_COLUMNS
        )
