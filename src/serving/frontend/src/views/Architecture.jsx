    ...
    return (
        <section id="view-model-architecture" className="view active">
            ...
            <ul className="arch-bullets">
                ...
                <li><strong>Loss:</strong> <code>MultiTargetLoss</code> — per head, from each position's <code>head_losses</code>: MSE for yards/points heads (all four K heads; DST points/yards allowed plus its sacks, INTs and fumble counts), Poisson NLL for QB/RB/WR/TE TD, INT and fumble counts and DST's four rare counts, and a zero-truncated NegBin hurdle for RB/WR/TE receptions on the attention NN (the dense NN's receptions head falls back to Huber) + optional BCE on the TD gate logit.</li>
                ...
                <li><strong>Early stopping:</strong> tracks the loss-weighted validation MAE (<code>val_mae_weighted</code>); the best <code>state_dict</code> is restored when training stops, whether patience expires or the epoch budget runs out.</li>
