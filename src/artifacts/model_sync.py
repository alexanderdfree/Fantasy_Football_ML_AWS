...
the smoke-test-validated ``stable`` artifact, falling back only to the
previous approved ``previous_stable`` artifact if available. The
...
Automatic destructive retention is suspended until collection is coordinated
with publication; see ``src/shared/artifact_gc.py``.
...
Fail-loud: if every approved manifest entry
...
#   {
#     "schema_version": 3,
#     "current":  {"key": "...", "sha7": "...", "bytes": int,
#                  "uploaded_at": "...", "smoke_passed": bool},
#     "stable":   <same shape> | null,    // last upload that PASSED smoke test
#     "previous_stable": <same shape> | null, // prior approved artifact
#     "previous": <same shape> | null,    // the prior current (forensics)
#     "history":  ["key0", "key1", ...]   // newest-first, capped at HISTORY_KEEP_N
#   }
# v2 stable remains trusted. Candidate-only v1/v2 manifests fail closed;
# operator promotion can validate an older artifact and establish approval.
MANIFEST_SCHEMA_VERSION = 3
HISTORY_KEEP_N = 5


class ManifestLockedError(RuntimeError):
    """An operator collector owns this position's publication manifest."""
...
def manifest_key(prefix: str, pos: str) -> str:
    return f"{prefix}/releases/v3/{pos}/manifest.json"


def legacy_manifest_key(prefix: str, pos: str) -> str:
    return f"{prefix}/{pos}/manifest.json"


def previous_protocol_manifest_key(prefix: str, pos: str) -> str:
    """Protected namespace used by the predecessor publication protocol."""
    return f"{prefix}/{pos}/releases/manifest.json"


def history_prefix(prefix: str, pos: str) -> str:
    return f"{prefix}/releases/v3/{pos}/history/"
...
def _load_manifest_with_etag(s3_client, bucket, prefix, pos):
    """Read a consumer manifest and the exact version consumed in one GET."""
    obj = _get_manifest_object(s3_client, bucket, prefix, pos)
    if obj is not None:
        return json.loads(obj["Body"].read()), obj.get("ETag")
    return _load_legacy_manifest_snapshot(s3_client, bucket, prefix, pos)


def load_legacy_manifest(s3_client, bucket: str, prefix: str, pos: str) -> dict | None:
    """Read-only migration source, preferring the protected predecessor protocol."""
    return _load_legacy_manifest_snapshot(s3_client, bucket, prefix, pos)[0]


def _load_legacy_manifest_snapshot(s3_client, bucket, prefix, pos):
    """Read the newest predecessor protocol, falling back only on missing keys.

    Both predecessors called their schema v3, so the physical namespace is the
    protocol discriminator. A main-protocol rollback has no epoch field: derive
    its fence from the exact manifest bytes so pre-training reads and migration
    capture the same token. Copying artifacts must preserve its source frontier.
    """
    obj = _get_manifest_object(s3_client, bucket, prefix, pos, previous_protocol=True)
    if obj is not None:
        body = obj["Body"].read()
        manifest = json.loads(body)
        if not isinstance(manifest, dict) or manifest.get("schema_version") != 3:
            raise RuntimeError("Invalid predecessor publication manifest")
        manifest = dict(manifest)
        frontier = manifest.get("publication_source")
        if frontier is not None:
            manifest["source_frontier"] = frontier
        if manifest.get("promotion_mode") == "rollback":
            if not isinstance(frontier, dict):
                raise RuntimeError("Predecessor rollback has no source frontier")
            manifest["rollback_epoch"] = "previous-protocol:" + hashlib.sha256(body).hexdigest()
            # The predecessor forbade ordinary completion of the same source
            # after rollback. Keep that rule across the first migration.
            manifest["rollback_source_barrier"] = frontier
        return manifest, obj.get("ETag")
    obj = _get_manifest_object(s3_client, bucket, prefix, pos, legacy=True)
    return (None, None) if obj is None else (json.loads(obj["Body"].read()), obj.get("ETag"))


def load_publication_manifest(s3_client, bucket: str, prefix: str, pos: str) -> dict | None:
    """Read the effective rollback fence before reserving a training intent.

    This is not a CAS snapshot. Publication/GC must use load_manifest_snapshot
    for the new namespace's ETag, then physically protect predecessor bytes.
    """
    manifest, _ = load_manifest_snapshot(s3_client, bucket, prefix, pos)
    return (
        manifest if manifest is not None else load_legacy_manifest(s3_client, bucket, prefix, pos)
    )


def load_manifest_snapshot(
    s3_client, bucket: str, prefix: str, pos: str, *, allow_locked: bool = False
) -> tuple[dict | None, str | None]:
    """Read content and its ETag in the same GET for conditional publication.

    An absent object has the generation ``None``. Other read failures propagate;
    publishers must never mistake denied or interrupted reads for a first write.
    """
    obj = _get_manifest_object(s3_client, bucket, prefix, pos)
    if obj is None:
        return None, None
    manifest = json.loads(obj["Body"].read())
    if manifest.get("gc_lock") and not allow_locked:
        raise ManifestLockedError(f"{pos} artifact collection lock is held: {manifest['gc_lock']}")
    return manifest, obj["ETag"]


def _get_manifest_object(
    s3_client, bucket: str, prefix: str, pos: str, *, legacy=False, previous_protocol=False
) -> dict | None:
    from botocore.exceptions import ClientError

    try:
        key = (
            previous_protocol_manifest_key(prefix, pos)
            if previous_protocol
            else legacy_manifest_key(prefix, pos)
            if legacy
            else manifest_key(prefix, pos)
        )
...
def write_manifest(
    ...
    *,
    ...
    """Atomically publish only if the generation read by this producer remains.

    ``None`` means create-only, never an unconditional overwrite. S3 conflicts
    propagate: retrying against a fresh generation would silently give a stale
    candidate precedence over a concurrent publisher or operator rollback.
    This orders publication transactions, not the start times of training jobs.
    """
    # A distinct nonce prevents ABA even when GC restores otherwise-identical
    # pointers or an operator promotes the same artifact again.
    ...
    s3_client.put_object(
        ...
        **({"IfMatch": expected_etag} if expected_etag is not None else {"IfNoneMatch": "*"}),
    )
...
def build_manifest(
    ...
    none). ``previous_stable`` retains the prior distinct approved artifact,
    independently of the bounded candidate history and failed uploads.
    """
    new_entry = {
        "key": new_key,
        "sha7": sha7,
    ...
    old = old_manifest or {}
    if old.get("gc_lock"):
        raise ManifestLockedError("Cannot build a publication from a GC-locked manifest")
    old_current = old.get("current")
    old_stable = old.get("stable")
    previous_stable = old.get("previous_stable")
    if smoke_passed and old_stable and old_stable.get("key") != new_key:
        previous_stable = old_stable
...
def _try_key(
    s3_client, bucket: str, key: str, dest: Path, expected_digest: str | None = None
) -> dict:
...
def _resolve_manifest_extract(s3_client, bucket: str, prefix: str, pos: str, dest: Path) -> dict:
    """Walk stable → previous_stable, extracting the first approved key that
    ...
    V2 ``stable`` entries remain approved during migration. Neither ``current``
    nor ``previous`` establishes approval; those slots deliberately include
    failed candidates. A candidate-only legacy manifest therefore fails closed.

    Each fall-through is logged with the grep-able tag
    ``source=stable|previous_stable`` so on-call can tell from CloudWatch
    ...
    seen: set[str] = set()
    for label in ("stable", "previous_stable"):
        entry = manifest.get(label)
        if not entry or not entry.get("key") or entry.get("smoke_passed") is False:
            continue
        if entry["key"] in seen:
            continue
        seen.add(entry["key"])
        try:
            # A corrupt archive may write some files before extraction fails.
            # Never mix that partial candidate with the next approved bundle.
            with tempfile.TemporaryDirectory(prefix="model-sync-") as staging:
                staged = Path(staging) / "models"
                r = _try_key(s3_client, bucket, entry["key"], staged, entry.get("sha256"))
                if dest.exists():
                    shutil.rmtree(dest)
...
def _sync_one(s3_client, bucket: str, prefix: str, pos: str, root: Path) -> dict:
    ...
    if result.get("key"):
        record_synced_model_key(root, pos, result["key"])
    if isinstance(result.get("manifest_etag"), str):
        (dest.parent / ".manifest-etag").write_text(result["manifest_etag"])
    return result


def record_synced_model_key(root: Path, pos: str, key: str) -> None:
    """Record the actual extracted key, including a legacy bootstrap source."""
    if pos not in POSITIONS:
        raise ValueError(f"Unsupported model position: {pos}")
    path = Path(root) / "src" / pos.lower() / "outputs" / ".synced_model_key"
    path.parent.mkdir(parents=True, exist_ok=True)
    fd, temporary = tempfile.mkstemp(prefix=".model-key-", dir=path.parent)
    try:
...
def refresh_position(
    ...
    re-download via the same stable→previous_stable fallback chain as
    ...
    try:
        for key in (
            manifest_key(prefix, pos),
            previous_protocol_manifest_key(prefix, pos),
            legacy_manifest_key(prefix, pos),
        ):
            try:
    ...
    if last_etag is None:
        # Skip only a key the boot sync actually installed. A first protected
        # manifest after legacy boot is a real update, not a bootstrap no-op.
        marker = paths["models"].parent / ".synced_model_key"
        try:
            local_key = marker.read_text() if marker.exists() else None
            current = load_manifest(s3_client, bucket, prefix, pos)
            if local_key and local_key == ((current or {}).get("stable") or {}).get("key"):
                return new_etag, False
        except (ClientError, OSError, ValueError) as error:
            print(f"[refresh] {pos} initial source check failed: {error!r}", flush=True)
            return last_etag, False
    ...
    record_synced_model_key(_repo_root(), pos, result["key"])
    new_etag = result.get("manifest_etag") or new_etag
    if isinstance(new_etag, str):
        with contextlib.suppress(OSError):
            (dest.parent / ".manifest-etag").write_text(new_etag)
...
# ---------------------------------------------------------------------------
#
# Offline builders publish complete schema-9 releases. Workers only download
# and verify immutable generation bytes; a missing release stays unavailable.

_PREDICTIONS_CACHE_DIR_REL = "data/serving_cache"


def sync_predictions_cache_from_s3() -> dict | None:
    """Install only a verified schema-9 serving release, retaining the last valid one.

    Legacy loose files and cache.tar.gz use incompatible protocols. A deployment
    must publish a new offline-built generation before cutting over its workers.
    """
    bucket = os.environ.get(_ENV_BUCKET, "").strip()
    if not bucket:
        return None
    prefix = os.environ.get(_ENV_PREFIX, "models").strip("/")
    destination = _repo_root() / _PREDICTIONS_CACHE_DIR_REL
    import boto3

    from src.artifacts.serving_snapshot import sync
    ...
    try:
        result = sync(
            ...
            destination,
            prefix,
            expected_dataset_id=os.environ.get("FF_DATA_RELEASE"),
            pinned_generation=os.environ.get("FF_SERVING_SNAPSHOT_GENERATION"),
        )
        return result if result is not None else {"files": 0, "missing_generation": True}
    except Exception as exc:
        print(f"[predcache_sync] generation unavailable; retaining local snapshot: {exc!r}")
        return {"generation_error": repr(exc)}


def upload_predictions_cache_to_s3() -> dict | None:
    """Compatibility no-op: only the offline builder may publish a serving release.

    A runtime worker has no pre-inference model-head CAS token. Uploading loose
    files or a legacy bundle here would bypass the complete-generation contract.
    """
    return None
