...
def _read_external_cache(
    ...
    if not cached.empty:
        return cached
    from src.data.providers.snapshot import provider_replay_mode

    if provider_replay_mode() is not None:
        # Retained dataset-v1 plans pin captures through FF_DATASET_ID without
        # a canonical .release marker. Their observed empty caches are truth too.
        return cached
...
def load_ff_opportunity(seasons: list[int], cache_dir: str = CACHE_DIR) -> pd.DataFrame:
    ...
    cached = _read_external_cache(path, (*keep, "_ff_opportunity_v2"), keep)
