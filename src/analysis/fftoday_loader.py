...
def _joined_cache_is_valid(cached: pd.DataFrame, min_match_rate: float, *, total_rows) -> bool:
    """Validate cached IDs and match coverage against the original projection count.

    Joined caches keep matched rows only, so the stored source-row denominator
    decides whether a stricter threshold sharing the rounded filename still
    passes. Literal-null IDs are stale data, never successful matches.
    """
...
def load_fftoday_with_gsis_id(
    ...
    *,
    ...
    if os.path.exists(cache_path) and not force_refresh and default_rosters:
        cached = pd.read_parquet(cache_path)
        if cached.attrs.get(_FETCH_COMPLETE_ATTR) is True and _joined_cache_is_valid(
