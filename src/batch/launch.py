...
def main():
    ...
    parser.add_argument(
        ...
        default="true",
        help=(
            "Publish one history row when all training jobs finish, even if "
            "this process stops waiting (true/false, default true)."
        ),
    )
    parser.add_argument(
        "--history-run-id",
        default=os.environ.get("FF_BENCHMARK_RUN_ID"),
        help="Unique history namespace; generated by default, pinned by CI per workflow attempt.",
    )
    parser.add_argument(
        "--collect-history",
        default="true",
        help="Also save the completed row locally after waiting (true/false); CI collects after rebasing.",
    )
