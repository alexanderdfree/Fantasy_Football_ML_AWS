"""Shared fixtures for RB tests — thin wrappers over src.shared.tests.position_fixtures.

Generic factories (``make_sim_df``, ``make_ranking_df``, ``make_tensors``,
``make_splits``, ``make_position_df``) now come from
``src/shared/tests/position_fixtures.py``.  Only RB-specific helpers remain
